...
- Each command has its dual — save the pair in src first, then run:
  ...
  - `audit/standards` — `npm run quantum:standards-audit` (`quantumStandardsAuditSuite()` — reverse demo RSA + inverse digit-zero/f→{p,q}/ratInv + reverse≠inverse + 10D coverage; alignment ≠ certification), then open `/en/quantum-encryption#quantum-standards-audit`
  ...
  - `challenge/ftl` — `npm run quantum:ftl-experiment-challenge` (`fasterThanLightExperimentChallenge()` / `quantumTechniquesAddressFtlExperiments()` in `src/fire/physics` — apparent-FTL experiment classes; no-signaling · plasma · Cherenkov · Hartman MODEL · OPERA · jets · teleport · tracks-classical-no-speedup; claySolvedByThisFold=0; physicalFtlSignalingClaimed=0), then `npm run docs:build`
  - `waves/ftl` — `npm run quantum:ftl-discovery-waves` (`ftlExperimentDiscoveryWaves()` / `sendWavesDiscoverRestFasterThanLightReuse()` in `src/fire/physics` — discovery waves at memoByRoot + `wavesAutoScaleCapacityAtNoCostOnReuse` velocity; composes sciences · directional · millennium · efficiency; physicalFtlClaim=0; gaps Alcubierre·CTC; NOT luminal), then `npm run docs:build`
