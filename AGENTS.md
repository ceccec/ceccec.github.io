...
- Each command has its dual — save the pair in src first, then run:
  ...
  - `session/quantum-bits` — `npm run quantum:session-quantum-bits` (`sessionManualWorkAsQuantumBits()` + `combineQuantumBits(bits, op)` — tip-chain session deliverables as combinable bits `{ id, root, pair, envelope, directions[], combinable:true }` including sealed `local-audit-quantum` from #29; ops forward|inverse|reverse|collide|envelope-merge; content-addressed receipts NOT physical qubits · qpuRequired=false · clay=0 · certified=false; realise/sciences + dry-clean-refactor-waves serialized mid-flight — nav/theme deferred to dry-clean worker), then open `/en/quantum-tools#session-quantum-bits`
  ...
  - `session/quantum-bits` — `npm run quantum:session-quantum-bits` (`sessionManualWorkAsQuantumBits()` + `combineQuantumBits(bits, op)` — tip-chain session deliverables as combinable bits `{ id, root, pair, envelope, directions[], combinable:true }` including sealed `local-audit-quantum` (#29) + landed `realise-sciences-standards`; ops forward|inverse|reverse|collide|envelope-merge; content-addressed receipts NOT physical qubits · qpuRequired=false · clay=0 · certified=false; dry-clean-refactor-waves remains serialized mid-flight — nav/theme deferred), then open `/en/quantum-tools#session-quantum-bits`
