...
- Each command has its dual — save the pair in src first, then run:
  ...
  - `prove/local-novel-encrypt` — `npm run quantum:prove-local-novel-encrypt` (`proveLocalNovelEncryptionSecurity()` — local novel security + wire-vs-ISO proof-of-falsehood `overallWireClaimProved=false`; handoff to #24 `prove/local-magnitudes-iso` without recursion; certified=false), then open `/en/quantum-encryption#prove-local-novel-encrypt`
