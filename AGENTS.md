...
- Each command has its dual — save the pair in src first, then run:
  ...
  - `prove/local-novel-encrypt` — `npm run quantum:prove-local-novel-encrypt` (`proveLocalNovelEncryptionSecurity()` — local novel-encryption security: inventory · allowlist · refuse · round-trip · timed reverse vs standards; productionReverseRefused=true · fieldHistory=none · certified=false), then open `/en/quantum-encryption#prove-local-novel-encrypt`
