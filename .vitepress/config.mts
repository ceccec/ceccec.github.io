...
import { computedSeo, jsonLdTemplate, siteConfig, siteNavigation, toGlagolitic } from './theme/lib/quantumMind'
...
// The Glagolitic locale (standard VitePress approach, third in the lang menu next to bg): the English
// navigation transliterated into the ninth-century script, its links pointed at /gla/. The /gla/ pages
// themselves are generated by scripts/generate-glagolitic.mjs (the English pages autotranslated).
const glaLink = (href: unknown): unknown =>
  typeof href !== 'string' || /^(https?:|#|mailto:|\/gla\/)/.test(href) ? href : href === '/' ? '/gla/' : `/gla${href}`
const glaNode = (node: any): any => {
  ...
  if (node && typeof node === 'object') {
    ...
    for (const [k, v] of Object.entries(node)) {
      ...
      else if (k === 'link') out[k] = glaLink(v)
      else if (k === 'message' && typeof v === 'string')
        out[k] = v.replace(/>([^<]+)</g, (_m, t) => `>${toGlagolitic(t)}<`).replace(/href="([^"]+)"/g, (_m, h) => `href="${glaLink(h)}"`)
