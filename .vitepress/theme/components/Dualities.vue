...
const tierName = (tier: number) =>
  bg.value
    ? { 3: 'основни', 5: 'структурни', 8: 'изразни' }[tier]
    : { 3: 'core', 5: 'structural', 8: 'expressive' }[tier]
...
const t = computed(() =>
  bg.value
    ? { eyebrow: 'всички дуалности · 3-5-8', lead: `${data.count} двустранни двойки в трите тиера на Фибоначи; всяка е истинска дуалност, защото редът има значение (ляво⇄дясно се различават).` }
    : { eyebrow: 'all dualities · 3-5-8', lead: `${data.count} two-sided pairs across the three Fibonacci tiers; each is a real duality because order matters (left⇄right differ).` },
)
</script>
