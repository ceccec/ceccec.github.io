...
// the Global Help widget. The provider SDK is imported only here and only when
// the user sends a message (client-side); the tools the model may call are
// restricted to the read-only concept commands. Security is by architecture:
// no secrets ship, the key stays in the caller's browser, output is text.
