...
<p v-if="params.link" class="diamond-link">
  This diamond indexes a real result — <a :href="`/en${params.link}`">open it →</a>
</p>
...
<p class="paper-nav">
  <a v-if="params.number > 1" :href="`/en/diamonds/d${String(params.number - 1).padStart(4, '0')}`">← Diamond {{ params.number - 1 }}</a>
  <a href="/en/diamonds/">All {{ params.total }} folders</a>
  <a v-if="params.number < params.total" :href="`/en/diamonds/d${String(params.number + 1).padStart(4, '0')}`">Diamond {{ params.number + 1 }} →</a>
</p>
