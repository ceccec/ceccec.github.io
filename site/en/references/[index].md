...
> (the bidirectional dual) of [paper {{ params.number }}](/en/papers/{{ params.paperId }}),
> a pointer that completes the forward/reverse pair the double torus always folds.

## Refers to

[**Paper {{ params.number }} — coordinate {{ params.coordinateIndex }} on cycle {{ params.generator }}**](/en/papers/{{ params.paperId }}) <span class="paper-chip-glyph">{{ params.glyph }}</span>
...
<p class="paper-nav">
  <a v-if="params.number > 1" :href="`/en/references/r${String(params.number - 1).padStart(3, '0')}`">← Reference {{ params.number - 1 }}</a>
  <a href="/en/references/">All {{ params.total }} references</a>
  <a :href="`/en/papers/${params.paperId}`">Paper {{ params.number }} →</a>
</p>
