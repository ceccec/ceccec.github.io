...
<p class="paper-nav">
  <a v-if="params.number > 1" :href="`/en/papers/p${String(params.number - 1).padStart(3, '0')}`">← Paper {{ params.number - 1 }}</a>
  <a href="/en/papers/">All {{ params.total }} papers</a>
  <a v-if="params.number < params.total" :href="`/en/papers/p${String(params.number + 1).padStart(3, '0')}`">Paper {{ params.number + 1 }} →</a>
</p>
