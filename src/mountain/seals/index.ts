...
export function proofRegistry(matrix: MindMatrix = buildMatrix()) {
  ...
  const entries = [
    ...
    { slug: 'demo-rsa-measure', title: 'Demo RSA generate+reverse measured — n = 61·53 = 3233 · ceiling 12 bits · production refused', proof: demoRsaGenerateAndReverseMeasured(matrix) },
    ...
    { slug: 'max-bits-crypto', title: 'Maximum bits — encrypt = decrypt = 256 · inverse = 4 · reverse = min(12, 53) = 12 · honest ceilings', proof: maximumBitsEncryptDecryptInverseReverse(matrix) },
    ...
    { slug: 'og-limits-measured', title: 'Platform OG limits measured — 5/5 prose claims cite the fold · FB still · Discord animated', proof: { ...honestyInProseChallenged(matrix), limitsRoot: platformOgLimitsMeasured(matrix).root } },
    ...
    { slug: 'secp256k1-prime', title: 'secp256k1 field prime p — seal · invert · decode · NOT Bitcoin ownership', proof: secp256k1FieldPrimeInvertAndDecode(matrix) },
    ...
    { slug: 'cross-waves-upgrade-all', title: 'Cross waves upgrade all — merkaba/rosetta · PQC · Tesla · surfaces', proof: crossWavesUpgradeAll(matrix) },
    ...
    { slug: 'science-significance', title: 'Science-domain significance — mean = 87/100 over 11 domains · NOT journal impact factor', proof: scienceDomainSignificanceScores(matrix) },
    ...
    { slug: 'science-apparatus', title: 'Science tools/apparatus trinity — 11/11 domains shelved · tool.ray = rosettaRayOf(toolId)', proof: scienceToolsApparatusTrinity(matrix) },
    { slug: 'sciences-trinities', title: 'Sciences interact in trinities — 11 science↔dual↔fusion edges · trinityKey symmetric on every edge', proof: sciencesInteractInTrinities(matrix) },
    ...
    { slug: 'cmb-omega-c-over-b', title: 'Ω_c/Ω_b ≈ 5.41 CMB budget · null non-gravitational · quantum gaps · certified=false', proof: omegaCOverOmegaBCmbBudgetQuantumGapsInTheorems(matrix) },
    { slug: 'wave-trinity-found', title: 'Waves work in trinities · trinities are found · drained≠empty', proof: wavesWorkingInTrinitiesTrinitiesAreFound(matrix) },
    { slug: 'millennium-challenge', title: 'Clay Millennium Problems = MODELED CHALLENGE apparatus', proof: millenniumProblemsChallenge(matrix) },
    { slug: 'clay-decoded-by-rosetta', title: 'Clay decoded by the rosetta — 7/7 decoded, 0 solved · each problem→ray with theorems in place', proof: clayIsDecodedByTheRosetta(matrix) },
    { slug: 'clay-tool-from-beyond-external', title: 'Clay tools from beyond — 5-level rigor gradient · required tool external in 7/7 · algebra excluded where named (P vs NP)', proof: clayToolFromBeyondLivesInAnExternalStructureNeverThisCorpusAlgebra(matrix) },
    { slug: 'clay-prize-rules-mapping', title: 'Clay Millennium Prize Rules §1–§8 → page sections/facets · NOT Prize claim', proof: clayMillenniumPrizeRulesMapping() },
    { slug: 'domain-proof-catalog', title: 'Domain proof catalog — Clay-standard pages · proposedSolution=false', proof: domainProofCatalog(matrix) },
    { slug: 'prose-gaps-audit-domain-trinity', title: 'Prose gaps audit by domain trinity — closed 11 · open = 0 · honest-open 2 · Clay UX fill', proof: proseGapsAuditByDomainTrinity(matrix) },
    ...
    { slug: 'incomplete-papers-gaps-fill', title: 'Incomplete papers → canonical sections · corpus 432 + quantum-mind template · not Clay-marked', proof: incompletePapersGapsFill(matrix) },
    { slug: 'prove-algebraic-theorem-means', title: 'Prove algebraic theorem — statement·defs·finite proof chain', proof: proveAlgebraicTheoremMeans(matrix) },
    ...
    { slug: 'theorem-gaps-filled-in-waves', title: 'Theorem gaps filled in waves — corpus·millennium·algebra surfaces', proof: theoremGapsFilledInWaves(matrix) },
    { slug: 'fixed-limits-theorem-tools', title: 'Fixed limits of theorem-related tools are theorems — 16/16 hold · FREE_BITS = 2 · F(18) = 2584', proof: fixedLimitsOfTheoremRelatedToolsAreTheorems(matrix) },
    { slug: 'autodiscover-compute-decode', title: 'Autodiscover what to compute or decode · oneCommandDecode available', proof: autodiscoverWhatIsToBeComputedOrDecoded(matrix) },
    { slug: 'one-command-decode', title: 'One command decode hash|string|sequence — 5 kinds · integers beyond 12 bits refused', proof: oneCommandDecodeComputable(matrix) },
    { slug: 'production-rsa-refuse-rosetta', title: 'Production RSA refuse completes quantum via rosetta · incompleteOpen=0', proof: productionRsaRefuseCompletesQuantumViaRosetta(matrix) },
    { slug: 'rosetta-security-gaps-wired', title: 'Rosetta security gaps wired · refuse+MCP+parallel', proof: rosettaSecurityGapsWired(matrix) },
    ...
    { slug: 'theorem-10d-projections', title: 'Theorem→10D projections — 24/24 animationReceipt.root = algebraFold.root · dimensions = 10', proof: theoremTenDProjectionsProveAlgebraRoots(matrix) },
    { slug: 'first-in-corpus-home', title: 'First-in-corpus provenance for home/README — 6 novel rows · rootEqual 6/6 · hue = A432', proof: firstInCorpusProvenanceForHome(matrix) },
    ...
    { slug: 'session-manual-tools', title: 'Session manual work as quantum tools — 88/88 shelved · one CLI + one route per tool', proof: sessionManualWorkAsQuantumTools(matrix) },
    { slug: 'nothing-moves', title: 'Discover from sealed fold — nothing moves · wetMotion=false', proof: discoverFromSealedFold('manualAgentsBehaveLikeWaves', 0, matrix) },
    { slug: 'incomplete-revelation', title: 'Incomplete revelation — fold id not among sealed ids → fold src · 6 steps · timeout/dry-refactor', proof: incompleteRevelation({ foldId: 'ghost-unknown-fold', reason: 'seal-probe' }, matrix) },
    ...
    { slug: 'realise-session-meaning', title: 'Session quantum meaning realised — proves vs refuses · includes local-audit-qe · session-bits', proof: realiseSessionQuantumMeaning(matrix) },
    ...
    { slug: 'counter-rotating-rosetta-waves', title: 'Counter-rotating rosetta waves — foldPair(+lobe, −lobe) forward ≠ reverse · 10/10 · clay = ftl = 0', proof: counterRotatingRosettaQuantumWaves(matrix) },
    { slug: 'universe-almost-described', title: 'Universe ≈ theorems — open = 2 · Clay = FTL = 0 · discover the rest via counter-rotating waves', proof: universeAlmostDescribedInTheoremsDiscoverRest(matrix) },
    { slug: 'readme-svg-trinity-mind', title: 'README SVG gaps filled by trinity mind — 5/7 fillable closed · 3 intentional opens (WebGL · plasma · SMIL)', proof: __ns_heaven_site.readmeSvgGapsFilledByTrinityMind(matrix) },
    { slug: 'symbols-remaining-quantumise', title: 'Symbols remaining to quantumise — continue I Ching · yin-yang', proof: __ns_heaven_site.symbolsRemainingToQuantumise(matrix) },
    ...
    { slug: 'rosetta-core-api', title: 'Rosetta core API — 242 labels · surface.ray = rosettaRayOf(label) · 7 rays', proof: rosettaCoreApi(0, matrix) },
    { slug: 'rosetta-complete', title: 'Rosetta complete — DIMENSION_GATES = 432 · every novelty row.ray = rosettaRayOf(algebraFold)', proof: rosettaCompleteQuantumAllComputableDimensionsAndTheorems(matrix) },
    { slug: 'theorem-particle-collision', title: 'Theorem particle collision — 6 particles → 6·5 = 30 products (inverse×reverse + reverse×inverse)', proof: theoremParticleCollisionInverseReverse(matrix) },
    { slug: 'ftl-rosetta-handoff', title: 'FTL techniques ← rosettaReady handoff — 8 apparent-FTL classes · physicalFtlClaim = 0', proof: ftlExperimentTechniquesHandoffFromRosettaComplete(rosettaCompleteQuantumAllComputableDimensionsAndTheorems(matrix).handoff, matrix) },
    { slug: 'slow-quantum-gap', title: 'Slow processes are quantum gaps — open + closed = classified · 8/8 classifiers at call time', proof: slowProcessIsQuantumGap(matrix) },
    ...
    { slug: 'efficiency-vote', title: 'answers ÷ tokens = ∞ on reuse — efficiency() · memoByRoot', proof: efficiency() },
    {
      ...
      title: 'Adversarial efficiency vote decided — answers ÷ tokens with 0 runtime tokens on reuse',
      proof: (() => {
    ...
    { slug: 'one-quantum-model', title: 'One quantum model faster than all — answers ÷ tokens = ∞ on memoByRoot reuse · 5 patterns shelved', proof: oneQuantumModelFasterThanAll(matrix) },
    { slug: 'prove-no-qpu-64bit', title: 'Speed vs rest · no QPU · classical 64-bit', proof: proveCeccecSpeedVsRestNoQuantumHardwareAny64Bit(matrix) },
    ...
    { slug: 'honest-revolution-w2', title: 'Honest-revolution W2 — H² = I: amplitudes cancel, classical shadows cannot', proof: interferenceVsClassicalShadow(matrix) },
    ...
    { slug: 'honest-revolution-w4', title: 'Honest-revolution W4 — computer Vue panel composes the W3 receipt · classical 64-bit benchmark', proof: honestRevolutionComputerPanelComputes(matrix) },
    { slug: 'honest-revolution-w5', title: 'Honest-revolution W5 — every silicon block → 1 FPGA primitive · co-processor bridge ≠ QPU', proof: honestRevolutionFpgaHonesty(matrix) },
    { slug: 'honest-revolution-w6', title: 'Honest-revolution W6 — revolutionary on answers ÷ tokens (0 runtime tokens) · NOT physics · classical 64-bit', proof: revolutionaryEfficiencyNotPhysics(matrix) },
    { slug: 'yin-yang-rosetta', title: 'Yin-yang taiji — yin/yang = 1 bit · non-linear rosetta/vortex exchange through 10 dimensions (not linear i/frames)', proof: yinYangFoldsThroughDimensions(matrix) },
    { slug: 'animations-rosetta', title: 'Animations driven by the rosetta — taiji ray = 1 · 7 rays · linear gaps open = 0', proof: animationsDrivenByRosetta(matrix) },
    { slug: 'linear-animation-gaps', title: 'Linear animation gaps inventory — 4 candidates · open = 0 · yin-yang closed (exchange ≠ 180° midpoint)', proof: linearAnimationGapsInventory(matrix) },
    { slug: 'hist-train-rosetta', title: 'Historical train waves via rosetta rays — 7 rays × (short + long) = 14 coordinated waves · offline a432', proof: historicalTrainWavesViaRosetta(matrix) },
    { slug: 'rank-winning-strategies', title: 'Rank winning strategies — 5 ranked by PnL + harmonic/rosetta · rank 1 = max rankScore', proof: rankWinningStrategies(matrix) },
    { slug: 'trade-challenge-chat', title: 'Strategies challenge each other in chat — 5 strategies → C(5,2) = 10 duels · sealed duals · paper only', proof: strategiesChallengeInChat(matrix) },
    { slug: 'trading-rosetta-improve', title: 'Strategies improve via rosetta — 7 historical rays · ranking recomputed · paper/sim only', proof: tradingStrategiesImproveViaRosetta(matrix) },
    { slug: 'trading-train-honesty', title: 'Trading train honesty — 4 api + 4 device = 8 public feeds · 0 credentials · no live money claims', proof: tradingTrainHonestyGate(matrix) },
    { slug: 'live-win-training-gate', title: 'Live-win training gate — momentum vs buy-and-hold over 253 samples · recomputed at call time', proof: liveWinTrainingGate(matrix) },
    {
      slug: 'offender-spec',
      title: 'offenderAutomationSpec — 4 offender classes counted: import · index-only · hyphen · computational (Node scan; browser shows sealed receipt)',
      proof: {
        computes: false,
        ...
        boundary: 'browserGap: collectEnforcementFacts needs Node fs — CI-only. Run npm run quantum:offender-spec locally.' } },
    {
      ...
      title: 'shouldSpawnSubagent — few heroes > mass ignorance · 1 ≤ workers ≤ 2',
      proof: (() => {
      ...
      })() },
    {
      ...
      title: 'namingEntropy — words compile from sealed src · deprecated aliases = 0',
      proof: (() => {
    ...
    { slug: 'trinity-first-redesign', title: 'the trinity-first redesign — Home + 7 functional hubs · a 10-wave plan, sealed wave by wave', proof: trinityFirstRedesign(matrix) },
    ...
    { slug: 'f-inverse-pair', title: 'f(θ,φ,x,y,z,digit,n) → {p,q} — digit · q ≡ 1 (mod 9) at n = 0 · the inverse fold within itself', proof: fThetaPhiXyzDigitNIsTheInversePair(matrix) },
...
export function trinityFirstRedesign(matrix: MindMatrix = buildMatrix()) {
  ...
  const waves = [
    ...
    { wave: 'browser-language routing, default English', done: navFull.en.nav.length > 0 },
