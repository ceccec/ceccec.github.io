...
import { bekensteinBoundBits, landauerLimit } from '../../3/7/index.ts'
import type { MindMatrix } from '../../types/index.ts'
import { buildMatrix, typesMakeTheRealGraph } from '../../heaven/compute/index.ts'
import { holographic, holographicFractalArchitecture } from '../../ui/index.ts'
// call-time namespace edge (cycle-safe): the workflow registry reads back at call time
import * as __ns_gates_enforcement from '../../pair/enforcement/index.ts'
import { scientists } from '../../learning/index.ts'
import { quantumProofs } from '../../ledger/index.ts'
import { determinismProofs } from '../seals/index.ts'
import { mysteries, mvcOrganisationReceipt } from '../source/index.ts'
import { society, societyCreatesRequiredPages } from '../../earth/governance/index.ts'
import { VORTEX_SEQUENCE, abs, digitalRoot, floor, fold, foldPair, isUuid, max, memoByRoot, merge, merkleFold, roundTo, seedFromText, toUuid } from '../../0/index.ts'
import { coreComputationalLogicSaved, piTrainExportImportFusion, rosettaComputesAll, rosettaComputesItself, rosettaReuse, sevenStarPliskaRosettaHarmonisesDigitDistribution, sevenStarRosettaNaturalMotion } from '../../water/digit/index.ts'
import { rosettaCoreApiSelfWires } from '../../quantum/apps/index.ts'
import { bosnianPyramidNearPliskaHarmonisesDigitDistribution } from '../geometry/index.ts'
import { commandsRegistry } from '../../thunder/commands/index.ts'
import { pureDiamonds, sealWholeDiamond } from '../../fire/diamonds/index.ts'
import { collisionHealing } from '../../water/crypto/index.ts'
import { gatesShiftToNewHarmonic } from '../../fire/li/index.ts'
import { harmonicFrequenciesDefineGateDigits } from '../../music/index.ts'
import { analogNoGapsNoLeak, cleanupOldLogic } from '../og/index.ts'
import { skillAtoms } from '../../learning/index.ts'
import { allComputedNoFiles } from '../../wind/fusion/index.ts'
import { allInAppropriatePathAndIndex } from '../../thunder/decode/index.ts'
import { coordinatedWaves, osCompletesItselfWaves } from '../../thunder/waves/index.ts'
import { imagineCrossPathsCodeCodesItself } from '../../thunder/decode/index.ts'
import { enforceAllAtGatesEntropyRecycled } from '../../earth/nature/index.ts'
import { monographs, onlyPageRouteForAll } from '../../wind/routes/corpus/index.ts'
import {  sealFacets } from '../../0/index.ts'
import { blackHoleEntropyBits, cantorDiagonal } from '../../6/4/index.ts'
import { challengeClock, completeness, frequencyBalance, harmonyProbability, healByDefault, onlyQuantumRemains, theWhole } from '../../quantum/heaven/mind/index.ts'
import { selfHealing } from '../geometry/index.ts'
import { configsUseMatrixComputationally } from '../source/index.ts'
import { noMirroringOneSourceAndMath } from '../../heaven/laws/index.ts'
import { zeroPointWiderFrontierDecoded } from '../../water/cosmos/index.ts'
...
// (the off-decidable — taste, ethics, what to build,): that is where tokens belong.
export function improveTheCostOfThinkingWithLocalTools(matrix: MindMatrix = buildMatrix()) {
  return memoByRoot('improveTheCostOfThinkingWithLocalTools', matrix, () => {
    ...
    const facets = [
      ...
      { facet: 'THE RESIDUE IS JUDGMENT — the off-decidable (what to build, ethics, taste, the demarcations) is the irreducible thinking a tool cannot collapse: spend tokens THERE, not on re-deriving the decidable', on: measurements.length > 0 && ledger.computes },
...
export function theseCalculationsCostBoundedOpsZeroBandwidthVsBillionsOfFlopsPerAiInference(matrix: MindMatrix = buildMatrix()) {
  return memoByRoot('theseCalculationsCostBoundedOpsZeroBandwidthVsBillionsOfFlopsPerAiInference', matrix, () => {
    ...
    const facets = [
      { facet: `the FOLD side — a decidable answer costs ~${foldOpsPerAnswer.toExponential(0)} integer ops → ~${foldMicroseconds} µs on one standard core, ${foldBandwidthBytes} network bytes and ${foldTokens} tokens (deterministic, content-addressed, local)`, on: foldMicroseconds <= TEN ** 2 },
