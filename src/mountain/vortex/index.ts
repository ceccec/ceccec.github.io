...
export function invertedSequenceLearnedFromErpax(matrix: MindMatrix = buildMatrix()) {
  return memoByRoot('invertedSequenceLearnedFromErpax', matrix, () => {
    ...
    return {
      ...
      boundary: earned('EXACT — recomputed locally from the sealed vortex:', facets, [{ facet: 'the adopted method verifies by local computation, never by trusting the partner corpus; the mirror extends to the void as self-mirror (m(0) = 0), so the reflected line is m of the whole forward line and each read rises from the void to its own head (0\\1 · 0\\9), the strokes are signs of steps, and the movie binding attests an EXISTING counter-rotating painter — no new physics, no new painter, no claim beyond the computed lines', on: ringStrokesFlip && pairsSumTen && involution }]) }
  })
}
