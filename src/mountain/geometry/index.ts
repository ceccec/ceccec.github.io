...
import { bb84, bernsteinVazirani, deutschJozsa, entanglementSwap, ghzMermin, interactionFreeMeasurement, noCloningWitness, quantumBatteryAdvantage, simon, teleportQubit } from '../../9/1/index.ts'
import { initialBearing, obliquityAtEpoch, phase, slip } from '../../6/4/index.ts'
import type { AnalogChannel, DualTorusTrinities, MindMatrix, TrinityAxis, TrinityPair, TrinityPhase, TrinityStep, MetatronCubeReport, MetatronEdge, MetatronNode, SacredGeometrySeal, SealLeaf } from '../../types/index.ts'
import { buildMatrix, circulateDoubleTorus, oneMathManyPresentations } from '../../heaven/compute/index.ts'
import { TORUS_LOBE_OFFSET, TORUS_RING_R, TORUS_TUBE_R_BASE, VORTEX_SEQUENCE, doubleTorusSurface, foldPair, gcd, isUuid, memoByRoot, merge, merkleFold, modUnits, sealFacets, seedFromText, toUuid, vortexNext, vortexPrev, roundTo, digitalRoot } from '../../0/index.ts'
import { dims, tenDimensionalAnimation } from '../../quantum/mountain/dimensions/index.ts'
import { LUNAR_NODAL_PERIOD_YEARS, ratStr } from '../../9/1/index.ts'
import { riseAzimuthDeg } from '../../3/7/index.ts'
import { greatCircleKm, setAzimuthDeg } from '../../5/5/index.ts'
import { MOON_ORBIT_INCLINATION_DEG } from '../../8/2/index.ts'
import { lunarStandstillDeclinationDeg } from '../../7/3/index.ts'
import { groupOrbit } from '../../4/6/index.ts'
import { REQUIRED_ANALOG_CHANNELS } from '../../heaven/atoms/index.ts'
import { doubleTorusMath, taxonomyIcons, torusUuid, crossFoldTrinity, selfAddressed, selfInteraction } from '../../fire/li/index.ts'
import { A432_HUE, GOLDEN_ANGLE, frequencyToLight, lobeHues } from '../../quantum/icons/index.ts'
import { cssIsIChingComputed, digitFolderMath, digitFolders, digitFoldersDoMath, ICHING_NUMBERS } from '../../earth/architecture/index.ts'
import { piTrainDiamonds, quantumFoldedBlockchains, selfBuild, streamSelfComplete } from '../../fire/diamonds/index.ts'
...
/**
 * THE QUINTIC THREEFOLD'S HODGE NUMBERS, COUNTED — closing the inventoried gap `cy-hodge-numbers`.
 *
 * stringTheoryMillenniumTheoremGapsInventory named it: "no sealed Calabi–Yau Hodge numbers h^{1,1},
 * h^{2,1} on a projective CY₃", and that was true — the corpus had the compact dimensions D−4 as a
 * structural count and mirror symmetry as a foldPair analogy, and neither is a Hodge number. Both
 * numbers of the quintic are exact COUNTS, so they are counted here rather than cited.
 *
 * h^{1,1} = 1 by the Lefschetz hyperplane theorem. For a smooth hypersurface of a projective space of
 * dimension ≥ 3 the restriction H²(Pⁿ) → H²(X) is an isomorphism, and H²(P⁴) is generated by the
 * hyperplane class alone. One generator, one (1,1) class.
 *
 * h^{2,1} = 101 by counting complex-structure deformations. A quintic in P⁴ is a degree-5 form in
 * 5 variables, so it has C(d + n, n) = C(9, 4) = 126 monomials. Two forms related by a linear change
 * of coordinates cut out the same variety, and GL(5, ℂ) has dimension 5² = 25 — which already absorbs
 * the overall scale of the defining polynomial. 126 − 25 = 101 deformations, and for a Calabi–Yau
 * threefold that count IS h^{2,1} (Kodaira–Spencer: H¹(X, T_X) ≅ H^{2,1}(X) via the holomorphic
 * 3-form). The identification holds BECAUSE the variety is Calabi–Yau, which is why the fold computes
 * the numbers only at the CY degree and reports the others as the counting function, not as Hodge
 * numbers of anything.
 *
 * CALABI–YAU IS A CONDITION ON THE DEGREE, NOT AN ADJECTIVE: a degree-d hypersurface in Pⁿ has
 * trivial canonical bundle exactly when d = n + 1 (adjunction, K_X = O_X(d − n − 1)). In P⁴ that
 * forces d = 5. Change either and the fold no longer claims a Hodge number.
 *
 * χ = 2(h^{1,1} − h^{2,1}) = 2(1 − 101) = −200, since a Calabi–Yau threefold's only free Hodge
 * numbers are those two.
 *
 * MIRROR SYMMETRY IS THE INVOLUTION ON THE PAIR — the corpus's own shape. The mirror of the quintic
 * has (h^{1,1}, h^{2,1}) = (101, 1) and χ = +200: the swap, and applying it twice returns the quintic.
 * That is stated as the arithmetic involution it is, and NOT as a proof of mirror symmetry, which is a
 * theorem about counts of rational curves that this fold does not touch.
 */
export function calabiYauHodgeNumbersOfTheQuinticCounted(matrix: MindMatrix = buildMatrix()) {
  void matrix
  const AMBIENT = 2 ** 2 // P⁴ — the projective dimension the quintic lives in
  const variables = AMBIENT + 1 // homogeneous coordinates
  const calabiYauDegree = AMBIENT + 1 // adjunction: K_X trivial iff d = n + 1
  const binomial = (n: number, k: number): number => {
    let r = 1
    for (let i = 0; i < k; i += 1) r = (r * (n - i)) / (i + 1)
    return round(r)
  }
  /** Degree-d forms in `variables` homogeneous coordinates, less the linear changes of coordinate. */
  const deformationsOfDegree = (d: number): number => binomial(d + AMBIENT, AMBIENT) - variables * variables
  const monomials = binomial(calabiYauDegree + AMBIENT, AMBIENT)
  const reparametrisations = variables * variables
  // h^{1,1} WAS THE LITERAL 1 CHECKED AGAINST 1 — the very shape this corpus spent a day purging, so it
  // is computed. Projective space has b_{2k} = 1 for 0 ≤ k ≤ n and b_odd = 0, and the Lefschetz
  // hyperplane theorem transports H² across only when the hypersurface has dimension ≥ 3. Drop the
  // ambient dimension and the theorem stops applying, and this stops being 1.
  const bettiOfProjectiveSpace = (n: number, degree: number): number =>
    (degree % 2 === 0 && degree / 2 <= n ? 1 : 0)
  const lefschetzTransportsH2 = AMBIENT - 1 >= 3 // dim X = n − 1, the theorem's hypothesis
  const h11 = lefschetzTransportsH2 ? bettiOfProjectiveSpace(AMBIENT, 2) : 0
  const h21 = deformationsOfDegree(calabiYauDegree)
  const chi = 2 * (h11 - h21)
  const mirror = { h11: h21, h21: h11, chi: -chi }
  const mirrorIsAnInvolution = mirror.h11 === h21 && mirror.h21 === h11
    && ({ h11: mirror.h21, h21: mirror.h11, chi: -mirror.chi }).h11 === h11
  // The counting function is not a constant — three neighbouring degrees, none of them Calabi–Yau,
  // so none of them is claimed to be a Hodge number; they are here so the formula can be seen to move.
  const neighbours = [calabiYauDegree - 2, calabiYauDegree - 1, calabiYauDegree + 1].map((d) => ({ d, deformations: deformationsOfDegree(d) }))
  const facets = [
    { facet: `h^{1,1} = ${h11} — Lefschetz transports H²(P^${AMBIENT}) across (dim X = ${AMBIENT - 1} ≥ 3: ${lefschetzTransportsH2}), and b₂(P^${AMBIENT}) = ${bettiOfProjectiveSpace(AMBIENT, 2)} — the hyperplane class alone`, on: lefschetzTransportsH2 && h11 === 1 },
    { facet: `h^{2,1} = ${h21} — C(${calabiYauDegree + AMBIENT},${AMBIENT}) = ${monomials} quintic monomials less dim GL(${variables}) = ${reparametrisations}`, on: monomials === 126 && reparametrisations === 25 && h21 === 101 },
    { facet: `the degree IS the Calabi–Yau condition — d = n + 1 = ${calabiYauDegree} in P^${AMBIENT} (adjunction), which is what makes the deformation count equal h^{2,1}`, on: calabiYauDegree === AMBIENT + 1 },
    { facet: `χ = 2(h^{1,1} − h^{2,1}) = ${chi} — a CY₃ has only those two free Hodge numbers`, on: chi === -200 },
    { facet: `mirror symmetry swaps the pair — (${mirror.h11}, ${mirror.h21}) with χ = ${mirror.chi}, and swapping twice returns the quintic`, on: mirrorIsAnInvolution },
    { facet: `the count is a function of the degree, not a constant — ${neighbours.map((n) => `d=${n.d}→${n.deformations}`).join(' · ')} (none Calabi–Yau, none claimed as Hodge numbers)`, on: neighbours.every((n) => n.deformations !== h21) },
  ].map((entry) => ({ ...entry, receipt: toUuid(`cy-quintic-hodge:${entry.facet}:${entry.on}`) }))
  return {
    ...
    statement:
      `The quintic threefold X₅ ⊂ P⁴ has h^{1,1} = ${h11} and h^{2,1} = ${h21}, so χ = ${chi}. The first is the Lefschetz hyperplane theorem; the second counts the ${monomials} degree-5 monomials in 5 variables less the ${reparametrisations} dimensions of GL(5,ℂ) that act on them, which equals h^{2,1} because the degree d = n + 1 makes the hypersurface Calabi–Yau. The mirror swaps the pair to (${mirror.h11}, ${mirror.h21}) and negates χ.`,
    // THE LIMITS COMPUTE — each is a predicate, not a promise.
    boundary: earned(
      'EXACT COUNTS on ONE variety — closing the inventoried gap cy-hodge-numbers, verified by its facets:',
      facets,
      [
        { facet: 'the mirror statement here is the arithmetic swap of the pair and nothing else — no rational-curve count is computed, so this is not a proof of mirror symmetry', on: mirror.h11 === h21 && mirror.h21 === h11 },
        { facet: `ONE variety, not Calabi-Yau moduli in general — the count is claimed only at the degree that makes the hypersurface Calabi-Yau, d = n + 1 = ${calabiYauDegree}`, on: calabiYauDegree === AMBIENT + 1 },
        { facet: 'no physics — the fold computes Hodge numbers and asserts no compactification of this threefold describes the world', on: Number.isInteger(h11) && Number.isInteger(h21) && Number.isInteger(chi) },
      ]) }
}
