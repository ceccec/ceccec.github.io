...
import * as __ns_up_computer from '../computer'
  ...
  return memoByRoot(`professionalResearchIndex:${Math.floor(at / 1000)}`, matrix, () => {
    const rows: ProfessionalResearchProgramRow[] = [
      ...
      },
      {
        ...
        mount: 'src/computer/substrate · src/computer',
...
export function researchIndex(matrix: MindMatrix = buildMatrix(), at = 0) {
  return memoByRoot(`researchIndex:${Math.floor(at / 1000)}`, matrix, () => {
    ...
    pushDomainRow(rows, 'hardware-drivers-research', 'Hardware drivers cross-probe research', 'src/computer', 'computations.cpu.gpu.memory.storage.cooperation', 'computerComputes(matrix) · npm run check:types', 'METAPHOR', driverResearch.researched, driverResearch.boundary, driverResearch.root)
    ...
    pushDomainRow(rows, 'computer-research', 'Computer system research', 'src/computer', '__ns_up_computer.computes', 'npm run mission:gate · /en/computer', 'METAPHOR', compR.researched, compR.boundary, compR.root)
    pushDomainRow(rows, 'computer-science-research', 'Computer science research', 'src/computer', '__ns_up_computer.science.computes', 'npm run check:types', 'DOCUMENTED', csR.researched, csR.boundary, csR.root)
