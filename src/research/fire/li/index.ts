...
import type {
  ...
  SelfInteraction, SelfState } from '../../../platform/wind/types'
...
import { quantumSitemap } from '../../../platform/wind/site'
...
import { DIMENSIONS, DIMENSION_NAMES, tenDimensionalAnimation } from '../../../quantum/platform/mountain/dimensions'
import {
  ...
  sexagesimal, fromSexagesimal, luoShu } from '../../../quantum/research/heaven/library'
...
import {
  ...
  dualTorusTrinities, merkaba, vortexMath, sacredGeometrySeal } from '../../../platform/mountain/geometry'
import { fuse64SealsMerkaba64Tetrahedra, quantumDoubleTorus, merkabasInDoubleTorus } from '../../../platform/mountain/topology'
...
import { everyLawProvesItsTripwire } from '../../../applications/thunder/verify'
import { allFormsAreTenDimensionalOrPurged } from '../../../platform/wind/fusion'
import { allComputedQuantumMathAnalog } from '../../../platform/wind/fusion'
...
import { commandsRegistry, commandsSavedInQuantumPairs } from '../../../applications/thunder/commands'
import { skillAtoms, mcpToolManifest, papers, quantumAcademy, threeWordWaves } from '../../../learning'
import { completeCorpus, siteNavigation } from '../../../platform/wind/routes/corpus'
...
import { pathIsMeaningDecodesCoordinates } from '../../../applications/thunder/decode'
import { rosettaGlagoliticGlobalKeyDecodeAll } from '../../../platform/wind/language'
import { endlessFusion, iChingFusionCompletesAll } from '../../../platform/wind/fusion'
import { endlessBackgroundMovie } from '../../../applications/thunder/movie/canvas'
import { warPaysTheForgerPrice } from './earth/world'
import { ogInOgWaves } from '../../../applications/thunder/waves'
...
export function enforcementTrinity() {
  return {
    ...
    boundary:
      'A declaration of the post-build enforcement surface. The runner lives in src/pair/enforcement (runEnforcementTrinity); src/pair/enforcement/script/cli/bootstrap/index.ts is the thin npm mount. The cross-check ranks the most-implicated subsystem to localise the root — a triage signal, not a proof that the top root is the only cause.' }
}
...
export function ancientTech(matrix: MindMatrix = buildMatrix()): AncientTechLens {
  ...
  const technologies: readonly AncientTech[] = (
    [
      ...
      { tech: 'Astrolabe', era: 'Hellenistic / Islamic', prefigures: 'coordinates folded onto a wheel', concept: 'concept.diamond.pi.train' },
...
export function buildEnforcementPipeline() {
  ...
  return {
    // The cross · fold · weave trinity runs via src/pair/enforcement/script/cli/bootstrap/index.ts
    // enforcement-trinity (a thin mount over src/pair/enforcement), wired into docs:build AFTER `vitepress build`
    // so it reads the real SSG dist (including the computed home at dist/index.html), not a stale one. The weave
    // wave verifies the declared subcommand is wired into docs:build alongside the bootstrap CLI entry.
