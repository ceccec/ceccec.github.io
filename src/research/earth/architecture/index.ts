...
import type { DigitFolderReport, DigitMath, DigitMathBinding, MindMatrix, PiTrainDiamond } from '../../../platform/wind/types'
import { buildMatrix, proofReport, reciprocity, verifyRoot } from './heaven/compute'
import { cellHomology, dualTorusTrinities, merkaba, areaPairs } from '../../../platform/mountain/geometry'
import { vortexMath } from '../../../platform/mountain/vortex'
...
import { developmentWaves, redistributeFoldersDryWaves } from '../../../applications/thunder/waves'
import { eachFolderIsMerkaba, quantumDoubleTorus, startIChingDoubleTorus } from '../../../platform/mountain/topology'
import { society } from '../governance'
import { selfSufficientWave } from '../../../platform/mountain/geometry'
import { frequencyTaxonomyTreeOfLife } from '../../../platform/mountain/topology'
import { metatronsCube, crossFoldTrinity } from './fire/li'
import { rosettaRayOf, ROSETTA_RAYS, ROSETTA_COMPUTATION_TYPES, type RosettaComputationType } from '../../../platform/water/digit'
import { staticPages, quantumSitemap, theoremScienceVisible } from '../../../platform/wind/site'
...
export function folderLaw() {
  return {
    ...
    rootAllowlist: {
      ...
      files: ['package.json', 'package-lock.json', 'pnpm-lock.yaml', 'pnpm-workspace.yaml', 'wrangler.jsonc', 'tsconfig.json', 'tsconfig.package.json', 'README.md', 'AGENTS.md', 'hero.svg', 'LICENSE', 'NOTICE', '.mcp.json'], // root config (npm + pnpm lockfiles, tsconfig for check:types, tsconfig.package.json for the @ceccec/double-torus kernel declarations), repo docs, the generated README hero, the LEGAL contract pair (Apache-2.0 LICENSE + NOTICE — external legal convention requires root placement, like the plugin manifests), and the Claude Code MCP wiring (.mcp.json — external editor contract)
...
// shim that only forwards (export { x } from '../../../quantum/research/heaven/quantum/heaven/y'), a per-function folder holding one symbol, fan-out that
