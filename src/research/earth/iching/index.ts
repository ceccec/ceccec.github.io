...
import * as __ns_up_fusion from '../../../platform/wind/fusion'
...
import * as __ns_thunder_waves_iching from '../../../applications/thunder/waves'
...
import { iChingImportExportTenD, iChingMotionAddsTheRest, iChingScalesComplete } from '../../../applications/thunder/decode'
import { startIChingDoubleTorus } from '../../../platform/mountain/topology'
import { hexagramReverseInverseDuality, nuclearHexagramFold } from '../../../platform/mountain/geometry'

export { BAGUA, iChing } from './heaven/core'
export { iChingDomainMap } from './heaven/balance'
export { iChingImportExportTenD, iChingMotionAddsTheRest, iChingScalesComplete } from '../../../applications/thunder/decode'
export {
  ICHING_NUMBERS,
  cssIsIChingComputed,
  ichingTokens,
  ...
  iChingFusionCompletesFolders,
  rosettaIChingTrinityPlacesAllTools } from '../architecture'
export { iChingFusionCompletesAll } from '../../../platform/wind/fusion'
export { startIChingDoubleTorus } from '../../../platform/mountain/topology'
...
export function hexagramOrbitCensusTwelveFoursEightTwos() {
  ...
  return {
    ...
    boundary: `Exhaustive computation over the 64 six-bit hexagrams — every count refutable by re-enumeration; the 20 agrees independently with the Burnside average sealed in decodingIChingAddsTheorems. Group theory on the classical pairing moves; no divination claim.` }
}
...
export function researchAroundFourThirtyTwoTheThreeTwentiesAreOneCountNotOneCause(matrix: MindMatrix = buildMatrix()) {
  return memoByRoot('threeTwentiesOneCount', matrix, () => {
    ...
    const facets = [
      ...
      { facet: `ONE COUNT, NOT ONE CAUSE — the three land on the single integer ${divisorCount} (${divisorCount} = ${orbits} = ${ladder}) yet only two share the mechanism of a forced lattice-count; the seam is NAMED (2 structural + 1 contingent), so the unity is arithmetic, not causal — made exact`, on: divisorCount === orbits && orbits === ladder && structuralTwenties + contingentTwenties === 3 },
