...
import { computesGate, floor, memoByRoot, merkleFold, toUuid } from '../../0'
...
export function quantumApplicationResearch(matrix: MindMatrix = buildMatrix(), at = 0) {
  return memoByRoot(`quantumApplicationResearch:${floor(at / (100 * 5 * 2))}`, matrix, () => {
...
export function quantumApplicationComputes(matrix: MindMatrix = buildMatrix(), at = 0) {
  return memoByRoot(`quantumApplicationComputes:${floor(at / (100 * 5 * 2))}`, matrix, () => {
