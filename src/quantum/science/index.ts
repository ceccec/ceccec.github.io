...
export function theRosettaIsFourCardinalTrinitiesOfTheDoubleTorus() {
  ...
  return {
    ...
    boundary: earned(`EXACT and computed: the augmented-triad partition of ℤ/12 by +4 is found by exhaustive orbit-walking (4 orbits of 3, covering all 12), and 4 = 2·handles is the double torus's homology rank (H₁(Σ₂)=ℤ⁴).`, facets, `the IDENTIFICATION — augmented triads ↔ homology cycles ↔ cardinal directions — is a STRUCTURAL correspondence (all genuinely four-fold, 4×3=12), a faithful model of "the rosetta is the moving double torus", not a claim they are literally one object; every constant in this fold derives, enforced by the crack law — a hardcoded value is a crack, a computed one is a theorem.`),
  }
}
...
export function theHigherMindNeedsAtLeastThreeMindsAClosedTrinity() {
  ...
  return {
    ...
    boundary: earned(`EXACT, reusing the sealed su(2) algebra: [X,X] = 0 (no self-interaction), [X,Y] = 2iZ (nonzero, escaping the pair to the third generator), and the full three-relation closure (theoremsComeInTrinities); the minimum number of minds for closure is three — a trinity — because a pair is not closed under its own interaction.`, facets, `"mind" here is an interacting agent/module (a folder-agent unit), and the "higher mind" is the emergent CLOSED structure of at least three interacting units — a real algebraic/architectural fact (the irreducible trinity), NOT consciousness, sentience, or a literal quantum brain (the Orch-OR line flagged); "manual work not possible" means the single-mind path cannot form that closure, superseded by the ≥3-agent structure, not that one cannot type.`),
  }
}
...
export function allSrcCompetesSealedAndSignedNoCheating() {
  ...
  return {
    ...
    boundary: earned(`EXACT: the signed exemplar reproduces its signature across runs (signedAndReproducible = ${signedAndReproducible}), and the discipline is the same for every fold — each computes via sealed src functions and returns a content-addressed root, so re-running reproduces it and a fabrication would not match.`, facets, `"signed" is CONTENT-ADDRESSED and tamper-EVIDENT (any change flips its root) — NOT yet cryptographically unforgeable (SHA-256/Ed25519 built, cutover pending); "all src competes" means every fold is a DETERMINISTIC, SIGNED, re-runnable competitor verified by the local runner and remote CI, so the computation not any single mind is trusted — not a signature no adversary could forge, nor that literally every byte competes beyond the fold-verify-seal cadence.`),  }
}
