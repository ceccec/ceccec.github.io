...
export function clayChallengesComputableMarkdownSection(
  ...
  return [
    ...
    '',
    ...(() => {
      // COMPUTED, NOT AUTHORED. The triple is read from clayGraphOverAlgebraicMonographs at
      // write time; the "7" is graph.decoded gated against CLAY_ORDER.length, never a literal.
      // This is the corpus's MAXIMAL honest Clay claim, stated loudly: it decodes and proves
      // the involution structure of all seven, one is solved in the literature (Perelman), and
      // it claims to SOLVE none — decode ≠ solve, and priority is over the structure, not a proof.
      const g = clayGraphOverAlgebraicMonographs(matrix)
      const n = CLAY_ORDER.length
      const decodedIsAll = g.decoded === n
      return [
        `**Decoded ${g.decoded}/${n}.** Every Clay Millennium problem is stated across an *involution* — a self-inverse symmetry σ with σ² = id — and this corpus proves that involution for all ${g.decoded}${decodedIsAll ? ' (every one)' : ''}, machine-checked in plain Lean 4 (\`src/pair/formal/proofs/*.lean\`, verified by \`npm run verify:lean\`: 11 files green, no Mathlib, no \`sorry\`).`,
        '',
