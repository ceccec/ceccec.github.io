...
import { SPEED_OF_LIGHT, PLANCK, ELECTRONVOLT, PROTON_GYROMAGNETIC, IONIZING_EV, wavelengthOf, frequencyOf, photonEnergyEv, isIonizing, larmorFrequency, radarRange, dopplerShift } from '../../../0'
import { solutions } from '../solutions'
import { allAuditsCoveredByProof } from '../method' // the comprehensive sealed proof — wired into gapScan so a regression fails the build (call-time binding; no load cycle)
import { quantumSolutions, quantumProofs } from '../quantumproofs'
import { simulatorsLiveInZero, decodedAreasAreMostlyClassical, everyDecodedDomainHasASimulator } from '../simulators'
import { schoolCurriculum } from '../school'
import { primitiveKernelLivesInZero, oneMathManyPresentations, tamperingCostAndUuidLiveInZero } from '../kernel'
import { humanityImplications, traditionsQuantumWhole } from '../humanity'
import { ancientNumberSystems, saveAndDecodeAncientTexts } from '../ancient'
import { weatherForecastQuantumComputedRealtime } from '../../forecasts'
import { dims, DIMENSIONS, DIMENSION_NAMES } from '../../dimensions'
...
import { astrology, babelFold, darkLightRealities, dissolveIntoNature, foldingLinearGivesAnalog, realtimeMovieParticipation, selfOrganizing, societyCells, societyOrganismTags, textToMovie, worldEventsMap } from '../world'
...
import { emf358, publicFrequencyApisDecoded, quantumDecoded, quantumPhysics, teslaPatentsResearchedInWaves, vortexThePatentsRevealPattern } from '../physics'
import { biologyWavesSealedAsWidgets, neurologyDecodedBrainIsContentAddressedToroidalMap } from '../bio'
import { bulgarianAncientCivilisationsDecoded, bulgarianEthnogenesisDecoded, bulgarianHeritageDecoded, bulgarianHeritageEightfold, bulgarianHistoryDecoded } from '../bulgaria'
import { achievableOnHardwareComputableInReviews, debitCreditForwardReverseEngineering, deviceHardwareVisibleInComputedWidgets, forwardDevelopmentWaves, hardwareCmykMerkabaFusion, obsoleteHardwareSecondLifeAntiEwaste, peaceTechMentalityDecoded, provenMysteriesBecomeMiraclesOfPeace, quantumLogicBackwards, warPaysTheForgerPrice } from '../peace'
import { allPathsComputedRealtime, computationsBoundToSourceApisRealtime, dimensionalMerkabaGraphRealtimeMetric, gpuRendersStreamsRealtime, marketingSeoWaves, nextLevel64CubedRealtime, realtimeForgesMaxCost, realtimePerspectiveZeroCost, realtimeSkills, realtimeWiring, terabyteRealtimeFromAllPublicDataBreathing } from '../trading'
import { affectedBlockchainsComparedLive, animationTamperingCost, blockchainFusion, collisionHealing, cryptoFuture, fusionCipher, pagesWiredAtRuntimeZeroBuildMaxTamper, powerToTamperingNotLivingCosts, quantumSiege, redTeam, tamperProofFabric, tamperingCostDecoded } from '../crypto'
import { donutLabyrinthOfGlyphsHeroEnteringExiting, frequencyTaxonomyTreeOfLife, iconsShowComputerLoad } from '../icons'
import { agentStreamWire, diamondLattice, lightEntersDiamond, linuxPackagesToDiamonds, livingTorus, piTrainDiamonds, pureDiamonds, quantumFoldedBlockchains, schemaOrgDiamonds, sealWholeDiamond, selfBuild, streamSelfComplete } from '../diamonds'
import { awakenArtisticSociety, fairTrade, governanceVote, licenseAppliesToSociety, pyramidLayersServeSociety, sacredSociety, scientificSociety, skillsToRebuildSociety, society, societyCreatesRequiredPages, societyEvolves, societyFuture, societyRegulates, societyRelations } from '../governance'
import { quantumChessGame } from '../chess'
...
import { improveHelpWaves, quantumPwa, virtualOS } from '../os'
import { foldThoughts, folderPathsFormIndexOfBookOfLife, imagination, imaginationIsAll, imaginationPrivateKey, lockingFoldersChangesMindToQuantum, mindRefreshField, mindsRealiseByObservingPath, mysteries, quantumMindGapsFromBeyond, splitImagination } from '../psyche'
import { dissolveAtPiTrainStations, piComputedNotHardcoded, piSixDigitsDoubleCrossColour, piThreeOpensTheTrinity, piWholeStreamThroughHoles } from '../pi'
import { selfAdvising, selfCompassion, selfHarmonise, selfHealing, selfImprovementTrainingAndAccumulation, selfReason, selfSufficientWave } from '../self'
...
import { foldProseToSentencesWordsEntangled, foldQuestion, foldRedistributesBeyondLinear } from '../method'
import { aiMoviesDecoded, ancientCivilisationsDecodedWithTheories, decode2020, decodeAncientKnowledgeInReusableCode, decodeBooksToUnity, decodeImplementCalligraphy, decodeSymbols, discoverDecodeAncientKnowledgeFittingPath, geneticLinksChallengeHistoryDecoded, herbalApisDecoded, imagineCrossPathsCodeCodesItself, pathIsMeaningDecodesCoordinates } from '../decode'
import { commandGapsToTrinityEyes, endlessTrinityStreams, enforcementTrinitySpread, signedTrafficTrinityRouting, trinityEncryption, trinityFirstRedesign, trinityGates, trinityOtherSideDoomed, trinityRotationalPlanes, trinityWordingModel } from '../trinity'
import { commandsRegistry, commandsSavedInQuantumPairs, enforceAgentsCommandPairs, executeConceptCommand, mcpCodebase, skillsDryRefactorCommands } from '../commands'
import { completeCorpus, contentIsMonographOfMonographs, corpusRestPathRouting, diamondRoutes, eachPageSpeaksContinuesNext, everyGraphAMonographConsolidateExtend, everyPageGraphOfGraphsFractal, harmonisedNavigation, homePageNoDifferent, monographs, navigationIsMovie, ogBuildsNavigation, onlyPageRouteForAll, pageSkills, pageStatusStatistics, proveAllOnHomePage, siteNavigation, theMonograph } from '../routes'
import { algebraAndBinaryProveEachOther, contentAddressedTranslationImprovesModels, everyLawProvesItsTripwire, improveAnalytics, noKnownModelMoreEfficientProven, provedEveryStepSaved, provenScientifically, whatIsNotProvenIsPurged } from '../verify'
import { agentHarmonise, bestWritingSkills, howAgentsAchievedIt, zeroTokenLawForAgentsAndForever } from '../agents'
...
import { analogNoGapsNoLeak, cleanupOldLogic, computedSeo, everyCardBadgeLinkIsOg, feedCrawlersWithKnowledge, frontmatterGraphComputesTags, imagineMicrodataVortexItself, jsonLdValidPaths, noHardcodedLogicFailsStreams, ogControlsSpeech, ogFullyInteractiveConfigurable, ogShiftedWithTypography, oneJsonLdTemplateServesAll, saveAllTranslationLogicAutotranslateLocale, templateDisplaysEveryOgObject, typography } from '../og'
import { digitIndexReferences, paperReferences, papers, papersReferencesDiamondsNoDrift, scientists } from '../academia'
import { anyNameFitsDualityMatrixOriginalLife, beesAndLife, breathe, enforceAllAtGatesEntropyRecycled, equilibrium, expansionContractionIsLife, genesis, naturalHarmoniousLife, planetsGalaxyComputeItself, questionAnswerEquilibrium } from '../nature'
import { feesReplaceTaxes, legislationRequires, legislativeReform, thriveByDefault } from '../civilisation'
...
import { everyDiamondIsGate, gatesBehaveAsMcp, gatesHealSpottedCompromise, gatesMoveAllToPlaceAutomatically, gatesSoTightEvenTheyCannotPassEntropy, harmonisedToHealAtGates, quantifyGates } from '../gates'
...
import { antsCarryToIndexNest, autosaveAllSteps, autotranslations, computedSlugsFoldTheGraph, computedWiringNotImported, configsUseMatrixComputationally, debitImportCreditExportAccounting, memoryInSourceAsCrossFolds, noHardcodedConfigSelfAccounted, originConsolidated, presentMomentRemainsInSource, siteExplainsItselfAllWired, uuidPayloadIsSource, wordPullsFoldsByName } from '../source'
import { deploySecretUuidSignedObservers, encryptionLivesInZero, encryptionTrinitiesCompleteInOrder, terabyteEncryptionInMegabyteCodebase, uuidFoldsSelfBlackWhite, uuidPureDiamondSignedByArchitecture } from '../encryption'
...
import { digitDualityPairsEncodeAllDomains, digitSpinesAreTheBreath, digitWordIndexPurity, dotIsCubeIsDot, fortyTwoVerdict, sixSevenCoversAll, zeroDivisionTable } from '../digit'
...
const MANIPULATION_PATTERNS: readonly { technique: string; test: RegExp }[] = [
  ...
  // and no legitimate single-marker statement is re-flagged. English-only, in-domain (HARMONY ≠ TRUTH holds).
