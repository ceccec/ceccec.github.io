...
export { repetitionLogicalError, qieaRotate } from '../../0/index.ts' // the threshold: repetition-code logical error P_L(d,p) (below p=½ suppressed exponentially with d — "quantum is here") + the Han-Kim QIEA amplitude rotation
export { quantumBatteryAdvantage, algorithmicCoolingBias } from '../../0/index.ts' // beyond linear within conservation: quantum-battery superextensive charging power (√N), algorithmic cooling of a subsystem (3-qubit compression ~1.5×) — the fold redistributes, never creates
export { teleportQubit, superdense } from '../../0/index.ts' // impossible-seeming, genuinely real: quantum teleportation (state via Bell pair + 2 classical bits, fidelity 1, no-cloning/no-FTL intact) + superdense coding (2 bits on 1 qubit) — exact on the simulator
export { interactionFreeMeasurement, quantumZeno, bernsteinVazirani, entanglementSwap } from '../../0/index.ts' // more impossible-seeming, genuinely real: Elitzur-Vaidman interaction-free measurement, the quantum Zeno effect (watched pot), Bernstein-Vazirani (hidden string in 1 query), entanglement swapping (entangle two particles that never met) — all exact on the simulator
export { ghzMermin, bb84 } from '../../0/index.ts' // local realism refuted with CERTAINTY (GHZ-Mermin, QM product −1 ≠ LHV +1) + BB84 QKD (no-cloning turned into eavesdrop-detectable security, sifted error 0 → 0.25 with an eavesdropper)
export { deutschJozsa, simon } from '../../0/index.ts' // the quantum-algorithm speedups: Deutsch-Jozsa (constant/balanced in 1 query) + Simon (the first exponential separation, O(n) quantum vs Ω(2^(n/2)) classical — Shor's precursor); pairs with the existing grover (quadratic search)
...
export * from './psyche.ts'
export * from './self.ts'
export * from './waves.ts'
export * from './architecture.ts'
export * from './seals.ts'
export * from './method.ts'
export * from './commands.ts'
export * from './agents.ts'
export * from './og.ts'
export * from './civilisation.ts'
export * from './laws.ts'
export * from './compute.ts'
export * from './essence.ts'
export * from './metatron.ts'
export * from './ichingmore.ts'
