...
export function counterdiffusionOnTheDoubleTorus() {
  ...
  return {
    ...
    boundary: 'SAFETY, FIRST AND LAST: this is a critique of models, NOT a model. Nothing here plans a dive — not this fold, not any fold in this repository. DOCUMENTED: circulation topology (two cycles); the Haldane-exponential ICD spike (a standard result, computed from the ledgered halftimes via Graham); the ICD literature (Lambertsen & Idicula 1975 · Doolette & Mitchell 2003); the correction history (ZHL-16A→B/C · Baker 1998 gradient factors · NEDU 2011 deep-stop reversal). REFUTED: that any topology forbids ICD — the body is the counterexample., and here it is literal: an elegant claim that says a documented injury cannot happen is the most dangerous kind of harmony. WITNESS (cited, not computed): and the body — which HAS this topology — provides the witness: ICD is documented (Lambertsen & Idicula 1975: skin lesions at constant pressure; Doolette & Mitchell 2003: inner-ear DCS from a He↔N₂ switch). The claim "ICD could never exist in a double torus" is REFUTED by observation, not by argument RANKING: the honest ranking: every dissolved-gas model is fitted and inaccurate — INCLUDING every fold in this repository, which contains NO validated decompression model and must never plan a dive' }
}
