...
export function theMerkabaDerivedItsMotionATheoremOfTetrahedralSymmetryNoAxiomAssumed() {
  ...
  const cyc = (v: number[]) => [v[2]!, v[0]!, v[1]!] // 120° rotation about the (1,1,1) body diagonal = cyclic coordinate permutation (a rotation, det +1)
  const rotZ = (v: number[], a: number) => [v[0]! * Math.cos(a) - v[1]! * Math.sin(a), v[0]! * Math.sin(a) + v[1]! * Math.cos(a), v[2]!] // a rotation about z
  const symmetryLooksStatic = key(tetraA.map(cyc)) === key(tetraA) // a SYMMETRY rotation maps the tetra to itself — the set is unchanged, so it looks static
  const nonSymmetryMovesNotStatic = key(tetraA.map((v) => rotZ(v, TAU / 8))) !== key(tetraA) // a NON-symmetry rotation moves the set off the cube corners — not static
  const motionIsATheoremOfSymmetry = symmetryLooksStatic && nonSymmetryMovesNotStatic // static-vs-moving is the symmetry group, not a free parameter
  ...
  const facets = [
    ...
    { facet: `STATIC-VS-MOVING IS THE SYMMETRY GROUP, A THEOREM — NOT A FREE AXIOM: a symmetry rotation (120° about a body diagonal = the cyclic coordinate permutation) maps the tetra to ITSELF, the vertex set unchanged, so it looks static (${symmetryLooksStatic}); a non-symmetry rotation (45° about z) moves the set off the cube corners, not static (${nonSymmetryMovesNotStatic}); so "when it moves it is not static" is governed by the tetrahedral symmetry group A₄ — derived, not an assumed parameter (${motionIsATheoremOfSymmetry})`, on: motionIsATheoremOfSymmetry },
  ...
  return {
    computes: facets.every((entry) => entry.on),
    tetraA, cubeVertices: cube.length, stellaIsCube, euler: V - E + F, symmetryLooksStatic, nonSymmetryMovesNotStatic, allDerived,
    facets,
