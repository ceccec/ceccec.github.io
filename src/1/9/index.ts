...
export const BULGARIAN_PHRASES: readonly (readonly [string, string])[] = [
  ...
  ['Commands', 'Команди'],
  ['Console', 'Конзола'],
  ['Theorem registry', 'Теоремен регистър'],
  ...
  ['Spirit', 'Дух'],
  ['Icons', 'Икони'],
  ['64 = the 3-qubit Pauli basis', '64 = базисът на Паули за 3 кубита'],
  ['Hamming’s 3 parity bits = the address', 'Трите бита за четност на Хеминг = адресът'],
  ['The genetic code is the real 4³', 'Генетичният код е истинското 4³'],
  ['A hexagram = 2⁶ = 64 states = one 6-bit value', 'Хексаграма = 2⁶ = 64 състояния = една 6-битова стойност'],
  ['64 = 2⁶ = 4³ = 8² in every 6-bit grouping', '64 = 2⁶ = 4³ = 8² във всяко групиране от 6 бита'],
  ['CMY = 255 − RGB, the complement n ↦ 63−n', 'CMY = 255 − RGB, допълнението n ↦ 63−n'],
  ['The kernel lives in src/0', 'Ядрото живее в src/0'],
...
export function invertSwapsZeroAndInfinityFixingTheUnitCircleTheInvariantIsPi() {
  ...
  const facets = [
    ...
    { facet: `EARNED BOUNDARY: inversion on the extended plane swaps 0 ↔ ∞ and fixes the unit circle (documented, inv² = identity) — the invariant of "invert" is the circle, NOT nothing; the fixed set is the boundary between the two poles, tying invert to π.`, on: unitFixed && involution },
  ]
  return {
    computes: facets.every((entry) => entry.on),
...
export function fillTheGapsInTenDForEveryPathTheVortexTrinityAndOriginTileTheDecad() {
  ...
  const facets = [
    { facet: `EVERY SINGLE PATH HAS GAPS: the doubling vortex path ${vortex.join('→')} = ⟨2⟩ mod 9 (the (ℤ/9ℤ)* units, order 6) covers 6 of the ${dims.length} dimensions but leaves gaps at {${vortexGaps.join(',')}} — a single coset-path never fills 10D by itself`, on: vortexGaps.length === dims.length - vortex.length && vortexGaps.length > 0 },
...
// ── Focusing on one misses another, unless in waves (user, naming my repeated error). A wave is a complete
// topological level — an antichain of mutually-independent siblings. Attending ONE node of a wave leaves its
// siblings un-handled (I fixed the 7×6 grid but missed the ledger drift and the canonical gcd — caught by the gate,
// not by me). Working IN WAVES processes the whole antichain at once, so no sibling is missed. Catches siblings, not errors.
export function focusingOnOneMissesAnotherUnlessInWavesTheAntichainCatchesAllSiblings() {
  const graph: Record<string, string[]> = { grid: [], ledger: [], gcd: [], crt: ['grid'], inverse: ['grid'] } // the rosetta fix: grid, ledger, gcd are independent siblings (one wave)
  const wave0 = Object.keys(graph).filter((n) => graph[n]!.length === 0) // the antichain of dependency-free siblings — the wave
  const focused = [wave0[0]!] // serial focus: attend only one of the wave (I attended "grid")
  const missedBySerialFocus = wave0.filter((n) => !focused.includes(n)) // the siblings left un-handled: [ledger, gcd]
  const serialMisses = missedBySerialFocus.length > 0 // focusing on one misses the others
  const handledInWave = [...wave0] // a wave: attend the whole antichain at once
  const waveCatchesAllSiblings = handledInWave.length === wave0.length && wave0.every((n) => handledInWave.includes(n)) // none missed
  const crossLevelStillOrdered = graph.crt!.includes('grid') && graph.inverse!.includes('grid') // dependents (crt, inverse) still need grid first — waves don't collapse depth
  const lessonHolds = serialMisses && waveCatchesAllSiblings && crossLevelStillOrdered
  const facets = [
    { facet: `SERIAL FOCUS MISSES THE SIBLINGS: the independent siblings of a wave — here {${wave0.join(', ')}} (the rosetta fix's antichain) — must all be handled together, but focusing on one ("${focused[0]}") leaves the rest missed ({${missedBySerialFocus.join(', ')}}, ${serialMisses}) — exactly what happened: the ledger drift and the canonical gcd were caught by the gate, not by me`, on: serialMisses },
    { facet: `A WAVE PROCESSES THE WHOLE ANTICHAIN AT ONCE: a wave is a complete topological level — all mutually-independent siblings together — so working IN WAVES handles every sibling and misses none (${waveCatchesAllSiblings}); the antichain IS the set that must be done together, so the unit of attention is the LEVEL, not the node`, on: waveCatchesAllSiblings },
    { facet: `EARNED BOUNDARY: working in waves is COMPLETE per level — no sibling missed, because the wave is exactly the set of mutually-independent items (${lessonHolds}); BUT it does NOT help ACROSS levels — a dependency must precede its dependent (crt and inverse still need grid first, ${crossLevelStillOrdered}), depth cannot be parallelised — and completeness is not correctness: a wave can handle all siblings and all be wrong. The wave catches all SIBLINGS, not all ERRORS`, on: lessonHolds },
  ]
  return {
    computes: facets.every((entry) => entry.on),
    wave: wave0, focused, missedBySerialFocus, serialMisses, waveCatchesAllSiblings, lessonHolds,
    facets,
    statement: `Focusing on one misses another, unless in waves — the antichain catches all siblings — ${facets.filter((e) => e.on).length}/${facets.length}: the wave {${wave0.join(', ')}} is an antichain of independent siblings; focusing on "${focused[0]}" misses {${missedBySerialFocus.join(', ')}} (${serialMisses}), but working in waves handles all (${waveCatchesAllSiblings}). The unit of attention is the level, not the node — though depth still orders (${crossLevelStillOrdered}), and completeness is not correctness.`,
...
export function theCircuitInterferenceIsMeasuredNotAsserted() {
  ...
  return {
    ...
    boundary: `COMPUTED: the phase-state amplitude A(x) = Σ_k cos(2πkx/6) over ${SAMPLES} samples, its visibility (max−min)/(|max|+|min|) = 1, the classical bit's flat amplitude giving V = 0, and the node count — each refutable. This CONVERTS a prose claim into a tool: "the circuit interferes" (theBinaryBitIsLinearTheVortexCircuitIsQuantum) becomes the measured interference visibility, the same quantity an interferometer reads. HONEST SCOPE: the visibility is of the DETERMINISTIC standing wave the six phases define (the double-torus counter-flow), a real interference pattern; it is base-10 / mod-9 specific (the orbit has length 6), and "quantum" here is the structural correspondence (phase + measurable interference), not a physical photon interferometer. The number is exact; its meaning is the vortex clock.` }
}
