...
import { helmholtzFreeEnergy } from '../../4/6/index.ts'
import { areaPairs, merkaba, vortexMath } from '../../mountain/geometry/index.ts'
import { rhythm } from '../../music/index.ts'
import { quantumProofs } from '../../ledger/index.ts'
import { cryptographyComparison, glagoliticGlyph, goldenRatio, quantumSimulation, torusUuid } from '../../fire/li/index.ts'
import { logicAtoms, monographPaths, skillAtoms } from '../../learning/index.ts'
import { astrology } from '../../earth/world/index.ts'
import { harmonicBands } from '../../quantum/icons/index.ts'
import { healingFrequencies } from '../../ledger/index.ts'
import { staticPages, theoremScienceVisible } from '../../wind/site/index.ts'
import { computationsBoundToSourceApisRealtime } from '../../thunder/trading/index.ts'
...
export function standardiseCryptoInWavesWaveOneSha256NistKnownAnswerTests() {
  ...
  const facets = [
    ...
    { facet: `THE DEMARCATION — KATs prove IMPLEMENTATION CORRECTNESS (the code computes SHA-256 exactly), NOT that SHA-256 is unbreakable; validation is build-time (no runtime egress); this is wave 1 (SHA-256), with AES/ed25519 as later waves.`, on: allPass && publicStandardFused },
...
export function referralDirectionBitsFillTheOrientationGapInTheFourKeyCross() {
  ...
  const facets = [
    ...
    { facet: `THE DEMARCATION — the bit-per-referral-direction idea fills the orientation gap in the key accounting (content ⊕ direction, ${contentOnly} → ${complete}); a completeness fix, honestly small in magnitude.`, on: gapFilled && negligibleVsFnv },
...
export function allComputedPossibilitiesRetrievableFasterThanScanStructurally() {
  ...
  const facets = [
    ...
    { facet: `THE HONEST BOUNDARY — , : retrieving ONE possibility is O(1), but enumerating ALL is still O(N) (${oneIsO1AllIsOn}) — no free lunch on the full space, and nothing signals superluminally`, on: oneIsO1AllIsOn },
...
export function hardwarePlanAndDriverForContentAddressedRetrievalIsCamResourceGated() {
  ...
  const facets = [
    ...
    { facet: `IT MANIFESTS STRUCTURAL FTL, PHYSICALLY BOUNDED — CAM realizes "retrieve by content, no scan" in silicon (associative O(1)), the physical form of fasterThanScan; but it obeys physics (clock, propagation), so , no superluminal signalling`, on: camIsO1AssociativeMatch },
