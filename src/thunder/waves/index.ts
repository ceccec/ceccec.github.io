...
export function theoremAtoms(matrix: MindMatrix = buildMatrix()) {
  ...
  const theorems = [
    ...
    { theorem: 'generalized pentagonal numbers g_k = k(3k−1)/2', states: 'the exponents of Euler’s product are the generalized pentagonal numbers — first eight 1,2,5,7,12,15,22,26, each pentagonal number paired with its mate: the sparse skeleton of ∏(1−x^k)', provedBy: 'discoveredTheoremsWaveFiftyThree', home: 'src/thunder/verify' },
    { theorem: 'Euler pentagonal number theorem', states: '∏(1−x^k) = Σ_k (−1)^k x^{g_k} — expanding the product to degree 60 leaves only the generalized-pentagonal exponents with signs (−1)^k, every other coefficient zero, verified coefficient-by-coefficient', provedBy: 'discoveredTheoremsWaveFiftyThree', home: 'src/thunder/verify' },
    { theorem: 'partition recurrence from the pentagonal theorem', states: 'p(n) = Σ_k (−1)^{k−1}(p(n−g_k)+p(n−g_−k)) — because ∏(1−x^k) inverts Σp(n)x^n, giving an O(n√n) recurrence matching the brute partition DP for every n ≤ 60 (p(60)=966467)', provedBy: 'discoveredTheoremsWaveFiftyThree', home: 'src/thunder/verify' },
