...
import { dopplerShift, earned, A432_OCTAVES, ROSETTA_RAYS, TAU } from '../../3/7/index.ts'
...
export function historicalTrainWavesViaRosetta(matrix: MindMatrix = buildMatrix(), at = 0) {
  return memoByRoot(`historicalTrainWavesViaRosetta:${floor(at / (100 * 5 * 2))}`, matrix, () => {
    ...
    const facets = [
      ...
      { facet: `NOT ad-hoc — schedule length === ROSETTA_RAYS.length · measured rayRuns.length=${rayRuns.length}`, on: rayRuns.length === ROSETTA_RAYS.length },
