...
import { initialBearing, obliquityAtEpoch, phase, slip } from '../../6/4/index.ts'
import { emergentDimensions, modelSeal } from '../balance/index.ts'
...
import { PROTON_GYROMAGNETIC, casimirEnergyPerArea, HUBBLE_CONSTANT_LOCAL, blackHoleEntropyBits, cantorDiagonal, quantumZeno } from '../../6/4/index.ts'
...
import { scaleColor } from '../../quantum/science/index.ts'
...
import { cssIsIChingComputed, ICHING_NUMBERS } from '../../earth/architecture/index.ts'
export { cssIsIChingComputed, ichingTokens, ichingTokensCss, scanCssForHardcoded, ICHING_NUMBERS } from '../../earth/architecture/index.ts'
...
import { humanDesignBodyGraphSvg, humanDesignBodyGraphSvgW7 } from '../sky/astronomy/index.ts'
...
import { fractalClockDur, realtimeWiring } from '../../fire/plasma/ball/index.ts'
...
import { counterRotatingRosettaQuantumWaves } from '../../quantum/apps/index.ts'
import { directionalTrinityForwardInverseReverse } from '../../water/stack/index.ts'
...
import { autoMovies8k, backgroundMovie, endlessBackgroundMovie, staticIsEventualGapInMovieFusion } from '../../thunder/movie/canvas/index.ts'
...
/**
 * DEMARCATION TOPIC SETS — read by the facets below instead of asserting a tier with `on: true`.
 *
 * Four facets stated the epistemic status of named topics ("FLAGGED — vitalism…", "UNCONFIRMED
 * HYPOTHESES (string/M-theory, loop quantum gravity…)") and carried `on: true`, so the tier was
 * a sentence and nothing could contradict it. The corpus already has the instrument: ONE sealed
 * DEMARCATION_REGISTRY that many folds read, and demarcate(topic) returns its tier.
 *
 * Now the facet CHECKS the prose against the registry. It fails in both directions — move a
 * topic between tiers and every fold naming it breaks; name a topic here with the wrong tier and
 * it breaks immediately. That is weaker than a physics computation and stronger than a sentence:
 * it does not establish that vitalism is abandoned, it establishes that this file and the
 * registry cannot disagree about it without something going red.
 */
const FLAGGED_THERMO = ['entropy = disorder', 'perpetual motion', 'syntropy', 'vacuum free-energy'] as const
const GRAVITY_UNCONFIRMED = ['string theory', 'loop quantum gravity', 'asymptotic safety', 'causal dynamical triangulations', 'MOND', 'quintessence', 'axions', 'WIMPs'] as const
const GRAVITY_PSEUDO = ['anti-gravity', 'electrogravitics', 'vacuum free-energy', 'quantum healing'] as const
const COSMO_UNCONFIRMED = ['eternal inflation', 'string theory', 'supersymmetry'] as const
const FLAGGED_BIO = ['vitalism', 'water memory', 'homeopathy', 'morphic resonance'] as const
const FLAGGED_NEUROMYTHS = ['we use only 10% of our brain', 'left-brain/right-brain personality', 'learning styles'] as const
const FLAGGED_FREQUENCY = ['cymatics', 'frequency medicine', '432 hz healing', '528 hz healing'] as const
const FLAGGED_SYNTHESIS = ['human design'] as const
...
export function gravityDecoded(matrix: MindMatrix = buildMatrix()) {
  ...
  return {
    ...
    boundary:
      'HONEST (a 2-agent sourced wave, LIGO/EHT/NASA/Nobel-cited): DOCUMENTED — GR’s field equations and its confirmations (Mercury 43″/cy, Eddington 1919, Pound–Rebka, GPS +38 µs/day, GW150914 Nobel 2017, GWTC-5.0 public release May 2026 with 390 cumulative events and GW250114 area-theorem test, EHT M87*/Sgr A*, lensing), r_s = 2GM/c² exact; the dark sector’s EVIDENCE (rotation curves, Bullet Cluster ~8σ, accelerating expansion Nobel 2011, ~68/27/5). KEY NUANCES: GR is corroborated, not "proven"/final (it breaks down at singularities and the Planck scale); "no confirmed quantum gravity" ≠ "no physics" (GR is a superb effective field theory; LIGO confirms its classical regime); GWTC-5.0 "second-generation" black holes are INTERPRETED from spin, not imaged; for dark matter the EVIDENCE is solid but the IDENTITY open, for dark energy the EXISTENCE is solid but the NATURE open — neither means "doesn’t exist". THE TWO-TIER DEMARCATION holds: string/LQG/asymptotic-safety/CDT, WIMP/axion, MOND and quintessence are UNCONFIRMED HYPOTHESES (rigorous, testable-in-principle), NOT pseudoscience; anti-gravity/electrogravitics, vacuum over-unity "free energy" (links zeroPointDecoded) and quantum/gravity "healing" (Chopra, Ig Nobel 1998) ARE pseudoscience (they violate conservation laws and have no reproducible evidence). Unproven ≠ pseudoscience.' }
}
...
export function readmeIsHomeHero10DAnimatedSvgInGithub(matrix: MindMatrix = buildMatrix()) {
  ...
  const fruitFusion = fruitOfLifeFusion(matrix)
  ...
  const facets = [
    ...
    { facet: 'merkaba 4-dir counter-rotation — rosetta-north/east/south/west · Earth poles-as-pyramid · opposite ω pairs · bothEarths ∧ counterRotatingRosettaQuantumWaves', on: allTheoremsProof && dualSpin && waves.counterRotating && earths.counterRotating && mk.counterRotating },
  ...
  return {
    ...
    flowerFruitPath: folPath,
    merkabaIsomorphic: dualSpin && mk.counterRotating,
...
export function readmeAnimatedIChingHeroBest10D(matrix: MindMatrix = buildMatrix()) {
  ...
  return {
    ...
    flowerFruitPath: home.flowerFruitPath === true,
    count: sealed.count,
    facets: sealed.facets,
    root: merge(matrix.root, sealed.root),
    statement:
...
export function iconsAreAnimatedToo(matrix: MindMatrix = buildMatrix()) {
  ...
  const pwaDual =
    ...
    && pwa.includes('wetStaticFoL=false')
    && !pwa.includes('<script')
  const facets = [
...
export function readmeSvgGapsFilledByTrinityMind(matrix: MindMatrix = buildMatrix()) {
  return memoByRoot('readmeSvgGapsFilledByTrinityMind', matrix, () => {
    ...
    const facets = [
      ...
      { facet: 'counterRotatingRosettaQuantumWaves ∧ merkaba ∧ bothEarths drive SVG dynamics', on: earths.counterRotating },
    ...
    return {
      ...
      filledCount: filled.length, // the gaps this fold reports as filled
      openCount: intentionalOpen.length, // and the ones it names as deliberately open
      count: sealed.count,
      facets: sealed.facets,
...
export function glagoliticLivingSmilSvg(opts: { size?: number; animate?: boolean; scale?: number } = {}): string {
  ...
  const cells = GLAGOLITIC_LETTERS.map((letter, i) => {
    ...
    return [
      ...
      `<text x="${x}" y="${y}" text-anchor="middle" dominant-baseline="central" font-size="${size / (8 + 5)}" fill="${fill}">${letter.glyph}</text>`,
      `</g>`,
    ].join('')
  }).join('')
  const dur = fractalClockDur(8)
...
export function torusKnotsSvg(opts: { size?: number; animate?: boolean; scale?: number } = {}): string {
  ...
  const R = size / (2 + 1 / PHI)
  const r = R / PHI
  const pairs: readonly { p: number; q: number }[] = [
    { p: FIBONACCI[1]!, q: FIBONACCI[2]! }, // (2,3)
    { p: FIBONACCI[2]!, q: FIBONACCI[3]! }, // (3,5)
    { p: FIBONACCI[3]!, q: FIBONACCI[4]! }, // (5,8)
  ]
  const steps = 8 * 8
  const pathFor = (p: number, q: number) => {
    const pts: string[] = []
  ...
  const knots = pairs.map((pair, i) => {
    const stroke = scaleColor(scale + i, { seedHue: A432_HUE, dark: true, L: 7 / 8, C: SVG_CHROMA })
    const sign = i % 2 === 0 ? 1 : -1
    const dur = fractalClockDur(6 + i)
    const spin = animate
      ? `<animateTransform attributeName="transform" type="rotate" from="0 ${cx} ${cy}" to="${sign * 360} ${cx} ${cy}" dur="${dur}" repeatCount="indefinite" additive="sum" data-spin-sign="${sign > 0 ? '+1' : '-1'}"/>`
      : ''
...
export function symbolsRemainingToQuantumise(matrix: MindMatrix = buildMatrix()) {
  return memoByRoot('symbolsRemainingToQuantumise', matrix, () => {
    ...
    const glagolitic = glagoliticLivingSmilSvg({ animate: true })
    const knots = torusKnotsSvg({ animate: true })
    ...
    const glagoliticLetters = (glagolitic.match(/data-glagolitic-name=/g) || []).length
    const knotMarks = (knots.match(/data-torus-knot=/g) || []).length
    const smilGlagolitic = (glagolitic.match(/<animateTransform|<animate /g) || []).length
    const smilKnots = (knots.match(/<animateTransform/g) || []).length
    ...
    const inventory: readonly { symbol: string; status: SymbolStatus; on: boolean; surface: string }[] = [
      ...
      { symbol: 'glagolitic-glyphs', status: 'sealed', on: /data-symbol="glagolitic-glyphs"/.test(glagolitic) && glagoliticLetters === GLAGOLITIC_LETTERS.length && smilGlagolitic >= 2 && /livingSmil=true/.test(glagolitic) && /wetStatic=false/.test(glagolitic), surface: 'glagoliticLivingSmilSvg←GLAGOLITIC_LETTERS∧glagoliticValue∧SMIL' },
      { symbol: 'torus-knots', status: 'sealed', on: /data-symbol="torus-knots"/.test(knots) && knotMarks === 3 && smilKnots >= 3 && /data-coprime="true"/.test(knots) && torus.alive && mk.counterRotating && /wetStatic=false/.test(knots), surface: 'torusKnotsSvg←FIBONACCI(p,q)∧livingTorus∧±ω SMIL' },
    ]
    ...
    const emitterBundle = vesica + bagua + metatron + yantra + tol + elements + om + futhark + alchemy + glagolitic + knots + hdBody
    const emittersOk =
    ...
    return {
      ...
      quantumised: sealed.ok && emittersOk && missing.length === 0 && partial.length === 0,
      inventory,
...
export function thermodynamicsEntropyDecoded(matrix: MindMatrix = buildMatrix()) {
  ...
  const facets = [
    ...
    { facet: `the honest floor under "zero-entropy computation" — deterministic, recompute-not-store, reversible computation APPROACHES the Landauer bound but never reaches zero and never beats it (irreversible erasure always costs k_B·T·ln2). The repo’s zero-entropy/zero-point COMPUTATION is this real reversible sense, NOT over-unity · measured landauer=${landauer}`, on: landauer > 0 },
    { facet: 'FLAGGED — "entropy = disorder" (a misleading analogy; some ordered states have higher entropy), perpetual motion of the 1st kind (violates energy conservation) and 2nd kind (violates the second law), "the 2nd law disproves evolution" (false — Earth is an OPEN system fed low-entropy sunlight), and "syntropy"/negentropy-as-a-life-force + vacuum free-energy (vitalism + perpetual motion)', on: FLAGGED_THERMO.every((topic) => demarcate(topic) === 'flagged') },
...
export function missingGlagoliticIsNotYetWiredOrObsolete(matrix: MindMatrix = buildMatrix()) {
  ...
  const facets = [
    ...
    { facet: `NOT-YET-WIRED — an ACTIVE concept whose translation is pending; it is on the roadmap, to be wired (the growing core) · measured pending=${pending}`, on: pending === 'not-yet-wired' },
    { facet: `OBSOLETE — a term NOT in the live set; retired or out of scope, intentionally absent (the purged rest) · measured obsolete=${obsolete}`, on: obsolete === 'obsolete' },
...
export function systemDevelopsItselfIndependently(matrix: MindMatrix = buildMatrix()) {
  ...
  const facets = [
    ...
    { facet: `every step is GATED green — the commit gate requires the I Ching complete (432 dimensions, 0 open, seal, taxonomy compliant), so the system can only advance in a complete, sealed state, NEVER into a broken one · measured gate.green=${gate.green}`, on: gate.green },
...
export function mindNeuroscienceDecoded(matrix: MindMatrix = buildMatrix()) {
  ...
  const facets = [
    { facet: 'the brain is a physical information-processing organ — ~86 billion neurons, ~10¹⁴ synapses; the action potential (Hodgkin–Huxley 1952, four coupled ODEs, Nobel 1963), chemical synapses (Ca²⁺-triggered SNARE exocytosis), LTP (Bliss–Lømo 1973 — the cellular model of memory, NMDA coincidence detection), place + grid cells (the inner GPS, Nobel 2014); connectomics from C. elegans (302) → FlyWire fly (139k) → the human H01 mm³ (~150M synapses)', on: life.decoded && neurons > 8e10 && synapses >= 1e14 },
      { facet: 'consciousness is carried as the FRONTIER — the measured biology is stated (neuron and synapse counts) and the competing theories are named as competing, which is the state of the field', on: life.decoded && neurons > 8e10 && synapses >= 1e14 },
      { facet: 'Orch-OR is carried as RIGOROUS-BUT-REJECTED — named and sourced beside the measured biology, and kept out of the sealed claims', on: life.decoded && synapses >= 1e14 },
    { facet: `FLAGGED neuromyths (pseudoscience) — "we use only 10% of our brain", strict left-brain/right-brain personality types, fixed "learning styles", most brain-training transfer claims, and "quantum brain/healing". REPO NOTE: this repo’s brain-as-content-addressed-torus / Hopfield–CA3 recall is a MODEL/metaphor (the reentry and holographic-brain analogs are contested), NOT a literal claim`, on: FLAGGED_NEUROMYTHS.every((t) => demarcate(t) === 'flagged') },
...
export function acousticsCymaticsDecoded(matrix: MindMatrix = buildMatrix()) {
  ...
  const facets = [
    ...
    { facet: 'FLAGGED — "cymatics/sound heals" and frequency-medicine (unproven), "432 Hz / 528 Hz are sacred/healing/DNA-repair frequencies" (numerology — the repo’s A432 keeps the ARITHMETIC real but flags the cosmic/healing claim), "cymatics proves sound creates matter / sacred geometry / the blueprint of creation", and Masaru Emoto’s water crystals (debunked, never blinded) and water-memory', on: FLAGGED_FREQUENCY.every((t) => demarcate(t) === 'flagged') },
