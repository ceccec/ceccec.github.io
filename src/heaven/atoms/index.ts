...
export function allAuditsCoveredByProof(matrix: MindMatrix = buildMatrix()) {
  ...
  const claims = [
    ...
    { facet: `sacred-math patent: applied math with a technical effect is NOT condemned · measured mathApplied.unlawfulIfGranted=${mathApplied.unlawfulIfGranted}`, on: !mathApplied.unlawfulIfGranted },
    { facet: 'product of nature (seed + isolated gene, US) is unlawful — Myriad / Funk Bros', on: natureSuch.unlawfulIfGranted },
    { facet: `genuine engineering (transgenic, non-naturally-occurring) is NOT condemned · measured natureEng.unlawfulIfGranted=${natureEng.unlawfulIfGranted}`, on: !natureEng.unlawfulIfGranted },
    ...
    { facet: `the genetic code IS math but NOT "sacred math" — the asserted premise is refuted · measured geneMath.codeIsMathematical=${geneMath.codeIsMathematical} · geneMath.basedOnSacredMath=${geneMath.basedOnSacredMath}`, on: geneMath.codeIsMathematical && !geneMath.basedOnSacredMath },
