...
import { buildMatrix, buildSequenceReducesComputations, maxEfficiencyCpuGpuMemoryStorageCooperation, verifyRoot } from '../heaven/compute'
...
export function computerResearch(matrix: MindMatrix = buildMatrix(), at = 0) {
  return memoByRoot(`computerResearch:${Math.floor(at / 1000)}`, matrix, () => {
    ...
    const rows: ComputerDriverRow[] = [
      { id: 'cpu', driver: 'sequential Markov + build sequence', home: 'src/computer/substrate', tier: cpu.tier, receipt: cpu.receipt },
      { id: 'gpu', driver: 'WebGL/canvas paint channels', home: 'src/computer/substrate', tier: gpu.tier, receipt: gpu.receipt },
      { id: 'memory', driver: 'memoByRoot + heap cap', home: 'src/computer/substrate', tier: memory.tier, receipt: memory.receipt },
      { id: 'storage', driver: 'merkle corpus + verifyRoot', home: 'src/computer/substrate', tier: storage.tier, receipt: storage.receipt },
      { id: 'bus', driver: 'receipt routing cpu↔gpu↔memory↔storage', home: 'src/computer/substrate', tier: bus.tier, receipt: bus.receipt },
      { id: 'display', driver: 'screen · media · viewport phase', home: 'src/computer/substrate', tier: display.screen.tier, receipt: display.receipt },
      { id: 'terminal', driver: 'TTY · console · CLI bootstrap', home: 'src/computer/substrate', tier: terminal.tty.tier, receipt: terminal.receipt },
      { id: 'power', driver: 'cpu+gpu draw metaphor', home: 'src/computer/substrate', tier: power.computes ? 'METAPHOR' : 'UNAVAILABLE', receipt: power.root },
    ]
