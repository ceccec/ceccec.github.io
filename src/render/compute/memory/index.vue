...
import { memoryPanelComputes } from './index'
...
const panel = shallowRef(memoryPanelComputes())
const driver = computed(() => panel.value.capstone.driver)
const seedParts = computed(() => ['Memory', driver.value.tier] as const)
const title = computed(() => panel.value.copy.title.en)
</script>

<template>
  <UiCardShell
    id="memory-driver-panel"
    class="driver-panel"
    component="Memory"
    movie-intensity="soft"
    :seed-parts="seedParts"
    :title="title"
    data-logic="src/render/compute/memory/index.ts"
    data-target="src/render/compute/memory/index.ts#memoryPanelComputes"
    data-topic="memory"
  >
    <header>
      <h2>{{ title }}</h2>
