  ...
  <section class="lens" :data-hexagram="ICHING_MASK.hexagram" :style="{ '--lens': ICHING_MASK.color }">
    ...
    <div v-if="analysis.crossAudit.vectors.length" class="lens__step">
      ...
      <ol class="lens__vectors">
        <li v-for="(v, i) in analysis.crossAudit.vectors" :key="i"><b>{{ tg(v.vector) }}</b> ×{{ v.count }} — {{ tg(v.defence) }}</li>
      </ol>
      <ul class="lens__guidance"><li v-for="(g, i) in analysis.crossAudit.guidance" :key="i">→ {{ tg(g) }}</li></ul>
    ...
    <div v-if="analysis.patent.isPatentText || analysis.patent.cores.length" class="lens__step">
      <h4>· patent subject-matter audit (§101 judicial exceptions)
        <span :style="{ color: analysis.patent.unlawfulIfGranted ? '#F00000' : ICHING_MASK.color }">{{ tg(analysis.patent.verdict) }}</span>
      </h4>
      <p v-if="analysis.patent.biggestViolator" class="lens__note">biggest violator: <b>{{ tg(analysis.patent.biggestViolator) }}</b></p>
      <ul class="lens__exceptions">
        <li v-for="(e, i) in analysis.patent.exceptions" :key="i"><b>{{ tg(e.category) }}</b> — {{ tg(e.verdict) }}<b v-if="e.unlawful" style="color:#F00000"> ⚠ unlawful if granted</b> <span class="lens__cores">[{{ e.cores.join(', ') }}]</span></li>
      </ul>
      ...
      <p class="lens__note">jurisdiction: <b>US</b> {{ tg(analysis.patent.us.verdict) }} · <b>EU</b> {{ tg(analysis.patent.eu.verdict) }}<b v-if="analysis.patent.jurisdictionDiverges" :style="{ color: ICHING_MASK.color }"> — they DIVERGE (EU Directive 98/44/EC Art. 5(2): an isolated sequence may be eligible even if identical to nature; US Myriad holds the opposite)</b></p>
      <p class="lens__note">{{ tg(analysis.patent.legalBasis[0]) }}. Educational, not legal advice — a granted patent is presumed valid until challenged; an engineered organism/cDNA, or applied math with a technical effect, can be eligible.</p>
    </div>
    ...
    <div class="lens__step">
      ...
      <ul class="lens__algebra"><li v-for="(a, i) in algebra" :key="i"><b>{{ tg(a.law) }}</b> <code>{{ a.expr }}</code></li></ul>
    </div>

    <div class="lens__step">
      <h4>4 · the real 10D — ten dualities, all matching true/false</h4>
      <ul class="lens__dual"><li v-for="(d, i) in tenD" :key="i" :class="{ on: d.coexist }">{{ d.coexist ? '✓' : '✗' }} {{ tg(d.axis) }}</li></ul>
    </div>
    ...
    <div v-if="analysis.alternative" class="lens__step">
      ...
      <p class="lens__alt">{{ tg(analysis.alternative) }}</p>
    </div>
