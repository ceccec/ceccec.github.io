...
const { tg, localize } = useLocale()
const itemHref = computed(() => (props.params.link ? localize(props.params.link) : props.params.link))
</script>
...
<template>
  <div v-if="params" :data-hexagram="ICHING_MASK.hexagram" :data-trigram="ICHING_MASK.glyph">
    ...
    <p class="paper-nav">
      <a v-if="params.number > 1" :href="localize(`/diamonds/d${String(params.number - 1).padStart(4, '0')}`)">← Diamond {{ params.number - 1 }}</a>
      <a :href="localize('/diamonds/')">All {{ params.total }} folders</a>
      <a v-if="params.number < params.total" :href="localize(`/diamonds/d${String(params.number + 1).padStart(4, '0')}`)">Diamond {{ params.number + 1 }} →</a>
    </p>
  </div>
</template>
