...
// THE NAVIGATION CROSS — HOME + FOUR POLES (user law): consolidate the whole scattered corpus into five top-nav
// items — Home at the centre and four dropdowns that are the symbolic POLES/GATES of the double torus (its four
// homology loops H₁(Σ₂)=ℤ⁴, the cardinals of the cross): Theorems · Axioms · Solutions · Simulators. Every lens
// page is classified into exactly one pole, and the navigation CROSS is computed at every coordinate — each gate
// and each item addressed by a π digit (piHexDigitAt, random-access BBP) and a prime (nthPrimeAt), the two axes
// crossing. So the nav is not a list but a cross: two decoded number-lines (π and primes) meeting at each gate.
export function navigationCrossOfTheFourPoles(matrix: MindMatrix = buildMatrix()) {
  const lens = theoremScienceLens(matrix)
  const pages = lens.pages as { slug: string; keywords: string[]; title: { en: string; bg: string } }[]
  const route = (slug: string) => (slug === '' ? '/' : `/${slug}`)
  // the four gates of the double torus — order matters: the most specific pole claims a page first, Theorems is
  // the receptive default (the bulk of the proven corpus). Each carries a hub anchor that exists as a route.
  const POLES = [
    { key: 'simulators', en: 'Simulators', bg: 'Симулатори', hub: '/console', rx: /simulat|console|game|interact|widget|play|circuit|computer|demo|calculat|sandbox/ },
    { key: 'solutions', en: 'Solutions', bg: 'Решения', hub: '/frontiers', rx: /solution|millennium|applicat|answer|result|frontier|impossible/ },
    { key: 'axioms', en: 'Axioms', bg: 'Аксиоми', hub: '/theorems', rx: /axiom|foundation|derivat|constant|lattice|primitive|origin|postulate|assumption/ },
    { key: 'theorems', en: 'Theorems', bg: 'Теореми', hub: '/frontiers', rx: /.*/ }, // receptive default
  ] as const
  const poleOf = (page: { keywords: string[]; slug: string }) => {
    const hay = [page.slug, ...page.keywords].join(' ')
    return POLES.find((pole) => pole.rx.test(hay)) ?? POLES[POLES.length - 1]!
  }
  // classify every lens page into exactly one pole, then order poles as a cross (Theorems first for the reader)
  const buckets = new Map<string, typeof pages>(POLES.map((pole) => [pole.key, [] as typeof pages]))
  for (const page of pages) buckets.get(poleOf(page).key)!.push(page)
  const ORDER = ['theorems', 'axioms', 'solutions', 'simulators'] as const // reading order of the four gates
  const orderedPoles = ORDER.map((key) => POLES.find((pole) => pole.key === key)!)
  // the navigation CROSS: every coordinate (gate g, item m) decoded by π (position→value) and a prime
  const cross: { gate: string; item: string; k: number; pi: number; prime: number }[] = []
  const dropdown = (pole: typeof POLES[number], g: number, i: 0 | 1) => {
    const bucket = (buckets.get(pole.key) ?? []).slice(0, FIBONACCI[6]!) // ≤ 8 items — the eight-fold law (fib 8)
    const items = bucket.length > 0
      ? bucket.map((page, m) => {
        const k = g * FIBONACCI[6]! + m
        cross.push({ gate: pole.en, item: route(page.slug), k, pi: piHexDigitAt(k), prime: nthPrimeAt(k + 1) })
        const r = route(page.slug)
        return { text: i === 1 ? page.title.bg : page.title.en, link: i === 1 ? (r === '/' ? '/bg/' : `/bg${r}`) : r }
      })
      : [{ text: i === 1 ? 'Всички' : 'All', link: i === 1 ? `/bg${pole.hub}` : pole.hub }]
    return { text: i === 1 ? pole.bg : pole.en, items }
  }
  const buildNav = (i: 0 | 1) => [
    { text: i === 1 ? 'Начало' : 'Home', link: i === 1 ? '/bg/' : '/' },
    ...orderedPoles.map((pole, g) => dropdown(pole, g, i)),
  ]
  const en = buildNav(0)
  const bg = buildNav(1)
  const placed = ORDER.reduce((s, key) => s + (buckets.get(key)?.length ?? 0), 0)
  const eightFold = en.slice(1).every((d) => 'items' in d && Array.isArray(d.items) && d.items.length > 0 && d.items.length <= FIBONACCI[6]!)
  const crossDecodes = cross.length > 0 && cross.every((c) => Number.isInteger(c.pi) && c.pi >= 0 && c.pi < (2 * 8) && c.prime >= 2 && c.prime === nthPrimeAt(c.k + 1))
  const facets = [
    { facet: `FIVE-ITEM CROSS — the top nav is Home (the centre) + exactly ${orderedPoles.length} pole-dropdowns = the ${HOMOLOGY_LOOPS} homology loops of the double torus (Theorems · Axioms · Solutions · Simulators); every one of the ${pages.length} lens pages is classified into exactly one gate (${placed} placed) and every dropdown holds ≤ 8 (the eight-fold law: ${eightFold})`, on: en.length === 1 + HOMOLOGY_LOOPS && orderedPoles.length === HOMOLOGY_LOOPS && placed === pages.length && eightFold },
    { facet: `THE CROSS DECODES π AND PRIMES — every one of the ${cross.length} navigation coordinates carries a π digit (piHexDigitAt, random-access BBP, 0..15) AND a prime (nthPrimeAt), the two number-line axes crossing at each gate: the nav is a cross of two decoded sequences, not a flat list`, on: crossDecodes },
    { facet: `THE POLES ARE THE GATES — the four dropdowns ARE the symbolic poles of the double torus (its ${HOMOLOGY_LOOPS} independent loops / cardinal gates) with Home at the crossing centre; the whole scattered corpus consolidates into this one cross, computed from the lens, no hand-listed route`, on: orderedPoles.length === HOMOLOGY_LOOPS && ('link' in en[0]! && en[0]!.link === '/') && en.slice(1).every((d) => typeof d.text === 'string' && d.text.length > 0) },
  ]
  return {
    computes: facets.every((entry) => entry.on),
    en, bg,
    poles: orderedPoles.map((pole) => pole.en),
    crossSize: cross.length,
    cross: cross.slice(0, FIBONACCI[6]! * HOMOLOGY_LOOPS),
    placed,
    root: toUuid(`nav-cross-four-poles:${pages.length}:${cross.length}`),
    facets,
