...
export function theBoundaryProseIsTheTokenSinkTerseAndEarnedBoundariesCutItMeasuredNotConvinced(root: string = enforcementScanRoot()) {
  ...
  return {
    ...
    boundary: `MEASURED: ${boundaryCount} boundary fields, avg ${avgBoundary} chars, ~${proseTokens} tokens; the fix is terse/earned() boundaries and inline (not spawned) analysis.: the number is the finding.` }
}
