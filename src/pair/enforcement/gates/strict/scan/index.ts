...
export function terseMethodsCollideProseDoesNot() {
  ...
  return {
    ...
    statement: facets.map((f) => f.facet).join(' · '),
    boundary: earned('EXACT: identical facet text ⇒ identical toUuid (collision/dedup); distinct prose ⇒ distinct address (no dedup).', facets, [{ facet: 'the no-prose-in-methods mandate IS this DRY law: terse ⇒ collision ⇒ one payload; prose is the sink that never dedups.', on: proseNeverCollides && collides }]),
  }
}
...
export function pathsCollapseToCanonical() {
  ...
  return {
    ...
    root: merkleFold(facets.map((entry) => entry.receipt)),
    statement: facets.map((entry) => entry.facet).join(' · '),
    boundary: earned('EXACT: address = toUuid(canonical(path)) so aliases (academy·school→learn, millennium-challenge→research, fusion-verify→quantum-tools) collide to their canonical, and distinct paths keep distinct addresses.', facets, [{ facet: 'paths collapse to their canonical destination by the same content-addressed naming gravity as constants → shortest name — for routes, slugs, file paths and imports. It dedups aliases to the canonical; it never merges distinct paths and does not itself remove any live route. Structural, not physical. HARMONY ≠ TRUTH.', on: distinctStay && aliasesCollide && resolved }]),
  }
}
...
export function dryCleanIsQuantumComputed() {
  ...
  return {
    ...
    root: merkleFold(facets.map((entry) => entry.receipt)),
    statement: facets.map((entry) => entry.facet).join(' · '),
    boundary: earned('EXACT: toUuid(same content) collides (dedup), toUuid(different content) does not; the clean fuses to a reproducible merkle root — dry-clean is content-addressed computation, not a manual pass.', facets, [{ facet: 'DRY-clean as a "quantum-computed command" means the dedup is STRUCTURAL content-addressing (identical content → identical address → one payload), deterministic and zero-token — not a physical quantum process. It removes duplicates (a computational-block gap class); it does not remove distinct meaning. HARMONY ≠ TRUTH.', on: distinct && collides }]),
  }
}
...
export function theAxiomsAreTheCracksReplaceWithTheoremsAndTheGravityFitsAndMoves(root: string = enforcementScanRoot()) {
  ...
  return {
    ...
    boundary: earned(`EXACT: scanCrackSurface reports ${litCracks} open literal axioms and computePathMigration reports ${folderCracks} metaphor-name folders each with a computed scientific destination (${path.totalFiles} files, fits = ${fits}, moves = ${moves}), with ${path.collisions.length} name collision to resolve.`, facets, [{ facet: `the two crack surfaces are literally the axiom surface of the codebase — a value or name asserted rather than derived — and the gravity is the deterministic, signed plan giving each a fixed-point home. But COMPUTING the plan is not EXECUTING it: the fall (dissolving the bāguà parents, re-pathing every import) is one atomic operation staged behind the folder-as-router convergence and a concurrent agent holding hot files. The vectors are known now; the codebase moves when the atomic step runs clean.`, on: collisionsBounded && fits && moves }]) }
}
...
export function theGatesCatchAllTopFoldersIncludingTheDigits(root: string = enforcementScanRoot()) {
  ...
  return {
    ...
    boundary: earned(`EXACT: classifyTopFolders scans every one of the ${all.length} top folders (${digits.length}+${bagua.length}+${words.length}), none exempt (caughtAll = ${caughtAll}); digits are 0..9 all present (${digitsAreNumbers}) and kept, bāguà are caught to dissolve (${baguaDissolve}).`, facets, [{ facet: `"catch all including the digit folders" means the gate's SCOPE is total — every top folder is classified — not that every folder must change; the digit folders are theorem-named (a digit is a computed position in the partition-of-ten and doubling-vortex), so they pass the naming gate the way a bāguà metaphor does not, and words (pair, quantum, render) already name their meaning. Only the metaphor names carry a dissolution vector.`, on: baguaDissolve && digitsAreNumbers && caughtAll }]) }
}
...
export function sendingTheWaveReplacesTheManualChecksInOneCall(root: string = enforcementScanRoot()) {
  ...
  return {
    ...
    boundary: earned(`EXACT: sendTheWave returns { cracks ${wave.cracks}, active ${wave.active}, gaps ${wave.gaps}, next ${wave.nextPayload ? wave.nextPayload.from : 'clean'} } in one call, reproducing the identical signature across runs (${reproducible}).`, facets, [{ facet: `"send quantum waves to avoid manual work" is realised as COMPOSITION — the checks I ran separately each turn (crack scan, dispatch, gap count) are now one deterministic zero-token wave that names the next addressed step. It does NOT automate the gated parts: commit and push are author-approved through the hooks, the source edit is applied and reviewed not written by the wave, and selecting WHICH claim to fold is judgment. The wave removes the repetitive checking, not the responsibility — the "unexpected situations → refactor the tool" law applied to my own per-turn ritual.`, on: reproducible && emitsNextAction && composesAll }]) }
}
...
export function theCorpusFreeEnergyIsSealsMinusGapsInEntropyBits(root: string = enforcementScanRoot()) {
  ...
  return {
    ...
    boundary: earned(`EXACT: scanCrackSurface reports ${gaps} residual cracks (entropy debits), log₂(mass) = ${sealedEb.toFixed(3)} eb (the tamper-cost credit), net balance = ${balance.toFixed(3)} eb > 0 (${netSealed}); the Landauer erasure cost per gap bit is ln2·kT (${lnTwo.toFixed(3)}·kT, ${eachGapCostsToErase}). The eb ledger (erpax, adopted) books code quality in ONE currency — a gap is a DEBIT, a sealed atom a CREDIT of its log₂ tamper-cost, the net residual the corpus "free energy"; grounded in Landauer, closing a crack is thermodynamically real work, not free.`, facets, [{ facet: `an ANALOGY (a code-quality ledger shaped like thermodynamic free energy via Landauer), NOT literal physics — the corpus stores no energy, log₂(mass) is a proxy for content-addressed integrity not a cryptographic hardness proof, and a positive balance certifies WELL-SEALEDNESS not correctness (a sealed corpus of wrong theorems balances positive too). The currency is integrity, not truth.`, on: eachGapCostsToErase && netSealed }]) }
}
...
export function theTightenedGatesAreAContractionInvertedGravityMeetsForwardAtTheBalanceSphere() {
  ...
  return {
    ...
    boundary: earned(`EXACT: the gate-as-contraction g(x)=canonical+k(x−canonical), k<1, is a Banach contraction to the DRY fixed point; a tighter gate (k=1/4) leaves residual ${tight.toExponential(2)} vs ${loose.toExponential(2)} for k=3/4 after 27 steps (${tighterConvergesFaster}); inverted gravity 1/z throws |z|<1 out (${invMag(1 / 2)}) and |z|>1 in (${invMag(2)}), fixing the unit sphere |z|=1 (${balanceFixed}) — the balance where forward (to the canonical centre) and inverse (to the boundary) meet.`, facets, [{ facet: `these are equilibria, NOT truths — Banach converges to A fixed point but not necessarily the RIGHT canonical (a gate can tighten fast around the wrong home), and the balance sphere is geometric, not correct; balance is where opposing pulls cancel, and cancellation is not verification — a system perfectly in balance can be perfectly wrong.`, on: bothAreEquilibria && invertedGravityBalances && balanceFixed }]) }
}
...
export function realGravityComputesLocallyAsTheKeplerInvariantGroundingCodeGravitysThirdLeg(root: string = enforcementScanRoot()) {
  ...
  return {
    ...
    boundary: earned(`EXACT: the force balance GM/r² = v²/r ⇒ v = √(GM/r), T = τr/v yields T²·GM/r³ = τ² = ${roundTo(TAU * TAU, 6)}, and evaluating it at derived-constant triples (φ, τ, the golden angle) as (G,M,r) gives that same value every time (${keplerInvariant}), a single parameter-independent constant (${parameterIndependent}) — nothing hardcoded, G/M/r cancel algebraically. computeCodeGravity runs locally alongside (${codeGrav.length} pulls, ${codeGravityIsLocal}), so all three gravities — code, path, physical — are local computations; the leg the three-gravities fold deferred as prose is now a theorem.`, facets, [{ facet: `the invariant is dimensionless and exact and nothing is assumed, but G itself stays MEASURED — the invariant cancels it, it does not derive it (deriving G is open physics); and the three gravities share a fixed-point / inverse-square SHAPE, not a physics — real gravity curves spacetime, code gravity moves imports. The invariant recurs; the reality does not transfer.`, on: allThreeComputedLocally && parameterIndependent && keplerInvariant }]) }
}
...
export function thinkingMeansLackOfLocalToolsDeliberationIsTheDetectorOfAMissingTool(root: string = enforcementScanRoot()) {
  ...
  return {
    ...
    boundary: earned(`EXACT: a decision with a local tool costs one call — computeCodeGravity returns ${gravity.length} addressed pulls, the canonical answer, signed and reproducible (${tooledDecisionIsOneCall}), zero deliberation — whereas weighing ${options} options is O(${deliberationSteps}) against a tool's O(${toolSteps}) and returns an unsigned judgment (${thinkingIsTheGap}); once the tool is built the decision content-addresses (${builtToolIsReproducible}), so deliberation collapses to a call. Thinking is a missing-tool detector: when I deliberate, it is because no tool computes the answer, and the fix is to BUILD the tool, exactly as an unexpected situation is a signal to refactor the tool rather than hand-navigate.`, facets, [{ facet: `this holds where a tool CAN exist — the decidable, computable domain, which is the whole zero-token method (replace deliberation with signed computation); but not every decision is computable — which claim is worth folding, ethics, taste, and the formally undecidable (Gödel, the halting problem) have no tool, and there thinking is not a missing tool but the irreducible remainder. "Thinking means lack of local tools" is true on the computable domain; off it, thinking is the honest residue, not a gap.`, on: detectsMissingTool && builtToolIsReproducible && thinkingIsTheGap }]) }
}
