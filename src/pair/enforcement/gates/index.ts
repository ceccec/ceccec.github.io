...
export const QUEUE_ROWS = [
  { wave: 'dev dynamic-route first mount', why: 'MEASURED (2026-07-24): after the TDZ fix, / mounts fully and dynamic routes MATCH (SSR title from params), all 250 modules evaluate OK — but on heavy dynamic routes the Vue app never attaches (no __vue_app__, no overlay, thread idle): the entry awaits a page-data/component promise that never settles on first mount', blocksCore: true, localOnly: true, toolExists: true, firstAction: 'time the dev page-data request server-side (vite logRequest on [path] params) and race-bisect the entry await chain (the class-8 tool)' },
