...
export const COMMAND_PLACEMENT_AUDIT_MAP: readonly CommandPlacementRow[] = [
  ...
  { fold: 'doNotProsePhysicalFtlItComputesTrue', pair: 'ftl/compute', currentBarrel: 'src/wind/research', bestPlace: 'src/wind/research', action: 'moved', reason: 'physicalFtl true|false · false⇒quantumise · agentsDoubtProse · immediateWire · CLI quantum:ftl-compute' },
...
export function queueNext(root: string = enforcementScanRoot()) {
  ...
  const missingIdentity = THEOREM_ATOM_SEED.filter((row) => !row.algebraicStatement && !extractAlgebraicStatement(row.states) && !/\(user[ ,:]|user law|user directive|user correction/.test(row.states) && /[=\u2261\u2264\u2265\u2260\u21D4\u21D2\u2208\u2209\u2282\u2286\u223C\u2245\u2248\u21A6\u2223]|\bmod\b|\biff\b| \| /u.test(row.states)).length
