...
export const QUEUE_ROWS = [
  { wave: 'dev dynamic-route content module', why: 'SHARPENED (2026-07-24, soft-nav instrument): app alive · route matches · $params.universal FULLY delivered client-side · UniversalPageTemplate registered — yet the markdown body renders EMPTY (vp-doc div childless): dev serves the not-found content component for dynamic routes while params/SEO compute; the gap is VP-dev content-module resolution against the custom automount', blocksCore: true, localOnly: true, toolExists: true, firstAction: 'trace server-side which content module dev serves for /quantum-tools (vite middleware log on the .md request) — then align the paths-file match with the content loader' },
