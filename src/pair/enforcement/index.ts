...
export function agentBashWorkflowsAreToolsSavedInSrc(matrix: MindMatrix = buildMatrix()) {
  ...
  const tools = [
    ...
    { station: '7', name: 'stale-bundle-clear', does: 'since 2026-09-14 each bundle is keyed on the files esbuild built it from (its metafile inputs), so an edit anywhere — scripts/ included — rebuilds it; a batch gate that still disagrees with an isolated fold run is NOT a stale cache. Clear it only to rule the cache out, then re-diagnose before touching any fold', steps: ['rm -rf .vitepress/cache/quantum-esbuild', 'npm run rosetta:diagnose'], scripts: [] as string[] },
...
export function dynamicStrategiesFromTheorems(matrix: MindMatrix = buildMatrix()) {
  ...
  return {
    ...
    boundary: `COMPUTED: every strategy row cites a real fold and folds live values into its receipt — refutable (win a race and the discover strategy's address changes). HONEST SCOPE: strategies are DECISION HEURISTICS derived from proven structure, not proofs themselves; the agent still judges fit (judgment is the off-decidable residue). "Realtime by the agents" = served at the workflow surface and recomputed per call at zero tokens; no claim that agents are compelled — the playbook informs, the gates enforce.` }
}
