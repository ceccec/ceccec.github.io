...
import * as __ns_up_water_digit from '../platform/water/digit'
...
import * as __ns_thunder_waves from '../applications/thunder/waves'
import * as __ns_up_language from '../platform/wind/language'
import * as __ns_up_earth_architecture from '../research/earth/architecture'
...
import type { AgentEducation, AgentLesson, ConceptSiteSection, McpTool, McpToolManifest, MindMatrix, StaticPage, SchoolCurriculum, SchoolLesson } from '../platform/wind/types'
import { buildMatrix, coherenceAnomaly, verifyRoot, coverage, reciprocity, matrixMemo } from '../research/heaven/compute'
import { cos, floor, fold, isUuid, max, merge, merkleFold, min, round, sin, toUuid } from '../0'
import { movie } from '../applications/thunder/movie/movielib'
import { AREA_ICONS, chakrasAura, efficiency, multidimensional } from '../quantum/icons'
import { atoms, conceptCommands } from '../research/heaven/atoms'
import { inverseShiftConsciousness, quantumSimulation, taxonomyIcons, universalLanguage } from '../research/fire/li'
import { rhythm } from '../music'
import { heartProtonAtomDecoded } from '../platform/mountain/geometry'
import { monographSliceFromRoute, ROUTE_ALIASES } from '../platform/wind/routes/automount'
import { homeHero, isServedRoute, localePath, pickLocale, displayText, quantumSitemap, staticPages, theoremScienceLens, theoremScienceVisible, siteDomainRegistry, applyDomainRegistrySidebars, dryCleanVitepressNavSidebarsFromDomainRegistry, folderMigrationDedupWaves, type LocaleName } from '../platform/wind/site'
import { componentGraph } from '../research/heaven/core'
import { realtimeWiring } from '../research/fire/plasma/ball'
import { toGlagolitic } from '../quantum/heaven/library'
import { mcpCodebase, mcpUsesVitepressSearch } from '../applications/thunder/commands'
import { completeCorpus, theoremRosettaSidebar, theRosettaReconfiguresVitepress } from '../platform/wind/routes/corpus'
import { diamondLattice } from '../research/fire/diamonds'
import { ROSETTA_RAYS, ROSETTA_RAY_HUB_SLUGS, rosettaDecodesUrlPath, rosettaRayOf, rosettaRayOfContent } from '../platform/water/digit'
...
import * as __ns_up_earth_iching from '../research/earth/iching'
import * as __ns_up_movie_canvas from '../applications/thunder/movie/canvas'
import { proofAcknowledgmentFormatSaved } from '../platform/wind/site'
import { quantumCoordinateNav } from '../research/fire/features'
import { openGraph } from '../quantum/icons'
...
import { analogSpeech } from '../quantum/icons'
import { displayAllWithFewEntropySaved } from '../ledger'
import { everythingFoldsMerkabaInfiniteStreams, merkabasInDoubleTorus } from '../platform/mountain/topology'
...
import { ogControlsSpeech } from '../platform/mountain/og'
...
export function navigation358(matrix: MindMatrix = buildMatrix()) {
  ...
  return {
    ...
    boundary: 'The 3-5-8 map derives at call time from the theorem-science lens + rosettaShelve; isServedRoute gates every item. Discovery hubs ≠ machine corpora. Addresses via the core API, not a parallel nav registry.' }
}
...
export function learningPortal(matrix: MindMatrix = buildMatrix()) {
  ...
  const sections = [
    { key: 'school', axis: 'by age', title: 'School', route: '/learn#school', kind: 'ladder', items: school.lessons.length, complete: school.complete, root: school.root, blurb: firstSentence(school.statement) },
    { key: 'academy', axis: 'by track', title: 'Academy', route: '/learn#academy', kind: 'courses', items: academy.courses.length, complete: academy.established, root: academy.root, blurb: firstSentence(academy.statement) },
    { key: 'paths', axis: 'by research', title: 'Math paths', route: '/learn#math-paths', kind: 'paths', items: paths.count, complete: paths.educates, root: paths.root, blurb: firstSentence(paths.statement) },
    { key: 'review', axis: 'by research', title: 'Peer review', route: '/learn#scientists', kind: 'challenges', items: review.count, complete: review.robust, root: review.root, blurb: firstSentence(review.statement) },
    ...
    { key: 'exam', axis: 'assess', title: 'Exam', route: '/learn#exam', kind: 'assessment', items: exam.count, complete: exam.graded, root: exam.root, blurb: firstSentence(exam.statement) },
    { key: 'agents', axis: 'by track', title: 'Agent education', route: '/learn#agent-education', kind: 'agents', items: agents.lessons.length, complete: agents.educated, root: agents.root, blurb: firstSentence(agents.statement) },
    { key: 'radar', axis: 'by research', title: 'Research radar', route: '/learn#deep-research-radar', kind: 'radar', items: radar.blips.length, complete: radar.radar, root: radar.root, blurb: firstSentence(radar.statement) },
...
export function certify(matrix: MindMatrix = buildMatrix()) {
  ...
  return {
    ...
    boundary:
      'HONEST: this is a COMPUTED credential/eligibility model — the certificate is the content-address fold of the completion roots of the education levels, and "editing rights" is the discipline that an agent completes (and understands) every level before editing the sealed model. It is NOT a runtime authentication or identity system: LLM agents are not cryptographically authenticated at commit time, so the gate it wires into is the build/commit gate, which can require certify().editingAllowed before edits land. "Certified" means each level fold reports complete and carries a recomputable credential, not an accredited human qualification.: passing the model\'s own assessment is self-consistency, not external proof. editingAllowed reports the live state — if any level is incomplete the right is withheld until the education is finished.' }
}
...
export function forge(matrix: MindMatrix = buildMatrix()) {
  ...
  return {
    ...
    boundary:
      'HONEST: "quantum simulations" are the project\'s DETERMINISTIC classical quantum-computer simulation (state-vector, Born-rule readout) and the domain simulators — not real quantum hardware. "Train" means RUN/practice the deterministic concept-commands (every command computes the same result every time), NOT gradient-descent learning — the commands are computed, not learned weights. "Forge tools" means define and save tools in src (the tools-in-src law), not manufacture. A safe practice ground (zero-cost, reproducible) for an agent to forge and rehearse before acting;' }
}
...
export function vitepressSidebar(matrix: MindMatrix = buildMatrix()) {
  ...
  return {
    ...
    boundary:
      'Nav aliases omit discovery keys; domain canonicals (incl. learn) get registry sidebars; /proofs is #61 domain-proof hub not a synonym.' }
}
...
export {
  ...
  rosettaComputesItself, rosettaDecodesUrlPath, rosettaRayOf, rosettaReuse, sevenStarRosettaNaturalMotion } from '../platform/water/digit'
export { rosettaCoreApi, rosettaCoreApiSelfWires, rosettaShelve, rosettaCoreApiSurface } from '../quantum/apps'
export { movieIsNeuroscienceComputation } from '../research/earth/life'
export { rosettaGlagoliticGlobalKeyDecodeAll, rosettaImprovesDictationAndDialects } from '../platform/wind/language'
export { rosettaIChingTrinityPlacesAllTools } from '../research/earth/architecture'
export { publicAstronomyNewsCitation } from '../research/earth/world'
