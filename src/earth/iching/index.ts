...
export function sevenRayRosettaIsTheSufficientArchitecture(matrix: MindMatrix = buildMatrix()) {
  return memoByRoot('sevenRayRosettaSufficient', matrix, () => {
    ...
    const facets = [
      { facet: `SEVEN SINGLE-STROKES — gcd(7, ${doublingPeriod}) = 1, so the Pliska {7/k} star is one continuous figure reaching every ray: seven is a sufficient coprime basis, not an arbitrary count`, on: singleStroke },
