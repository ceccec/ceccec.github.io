...
export function folderLaw() {
  return {
    ...
    rootAllowlist: {
      ...
      files: ['package.json', 'package-lock.json', 'pnpm-lock.yaml', 'pnpm-workspace.yaml', 'wrangler.jsonc', 'tsconfig.json', 'tsconfig.package.json', 'README.md', 'AGENTS.md', 'hero.svg', 'LICENSE', 'NOTICE', '.mcp.json', 'CITATION.cff'], // root config (npm + pnpm lockfiles, tsconfig for check:types, tsconfig.package.json for the @ceccec/double-torus kernel declarations), repo docs, the generated README hero, the LEGAL contract pair (Apache-2.0 LICENSE + NOTICE — external legal convention requires root placement, like the plugin manifests), the Claude Code MCP wiring (.mcp.json — external editor contract), and CITATION.cff (the Citation File Format is defined to sit at repository root; GitHub's "Cite this repository" widget reads it only from there, so it cannot move into src/)
