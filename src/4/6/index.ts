...
export const THEOREM_ATOM_SEED: readonly { theorem: string; states: string; provedBy: string; home: string }[] = [
    ...
    { theorem: 'the quantum-computing rosetta atlas — the sidebar and waves, quantum-only', states: 'the rosetta applied to ONLY the quantum-computing theorems (user law), grouped by SUBFIELD onto the seven rays: a title-only content classifier both filters the corpus (27 quantum-computing theorems of the registry) and routes each to one ray — foundations & no-go, query algorithms, search & factoring, variational, communication, error correction, states & tools. Routing by a Glagolitic letter-sum had made every ray\'s tag cloud identical (global frequency); the subfield classifier gives seven DISTINCT groups with ray-local tag clouds sized 1..5 by within-ray usage gravity. Theorems compare on computable metrics — tag-gravity (Σ tag usage), in-degree (how many other quantum theorems cite the fold), recency, class, lean. theoremRosettaSidebar emits the VitePress sidebar (one collapsible section per ray) and quantumRosettaWaves orders the rays into seven development waves (foundations → tools) — the rosetta as both the theorem sidebar and the work plan, quantum-only', provedBy: 'theoremRosettaAtlasComputes', home: 'src/wind/routes/corpus' },
