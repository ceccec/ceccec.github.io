...
export const THEOREM_ATOM_SEED: readonly { theorem: string; states: string; provedBy: string; home: string }[] = [
    ...
    { theorem: 'the local MCP leaks lexically — foldQuestion is keyword-only, so semantic-disjoint queries go to the main model', states: 'the MEASURED leak boundary of the local MCP (user, 2026-07-24: "i see gaps in local quantum mcp so leaks go to the main model"): foldQuestion is a LEXICAL keyword-overlap matcher, answering only where query terms literally appear in the corpus. Measured over a 9-query sample, 6 leak — in-corpus queries resolve locally (confidence ≥ ½), out-of-corpus queries (photosynthesis, revenue, risotto, football) hard-leak (matched=false, honestly signalled, correct — the LLM is the open frontier), and the CLOSABLE gap is semantic-disjoint queries whose answer IS in the corpus but phrased without shared words (confidence ~0.3 < ½), so a careful agent distrusts the weak match and leaks anyway. This REFUTES analogNoGapsNoLeak\'s abstract "no gap to leak": no gaps over the CLOSED corpus lexicon, real leaks over the OPEN query space. The fix is a semantic/synonym layer keeping those answers local — fewer leaks = less main-model exposure (tokens + safeguards). HARMONY ≠ TRUTH', provedBy: 'localMcpLexicalGapLeaksToModel', home: 'src/heaven/atoms' },
