...
export const THEOREM_ATOM_SEED: readonly { theorem: string; states: string; provedBy: string; home: string }[] = [
    ...
    { theorem: 'the lens improves itself using the rosetta — 9 orphans to 1 frontier', states: 'the discovery lens does not merely report the undiscoverable; it computes, from the rosetta, the cross-link that surfaces each orphan, verifies its own blind spot shrinks, and names the irreducible residual (user law). For each of the 9 undiscoverable orphans (in-degree 0, low tag-gravity) the rosetta supplies its RAY HUB — the highest-gravity sibling in the same subfield — as a "discover via the rosetta" link; 8 orphans gain one and leave the undiscoverable set, so the lens improves ITSELF from 9 to 1. What survives is exactly the orphan in a SINGLETON subfield (variational, one member, no sibling to link to) — irreducible by linking alone, it names the frontier: where a new theorem must be developed. Self-improving to a fixpoint, honest about the residual it cannot fix without developing the missing sibling', provedBy: 'quantumLensImprovesItself', home: 'src/wind/routes/corpus' },
