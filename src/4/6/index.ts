...
export const THEOREM_ATOM_SEED: readonly { theorem: string; states: string; provedBy: string; home: string }[] = [
    ...
    { theorem: 'standard scientific format mapping completed — theoremAsStandardScientificPaper maps any fold to the full IMRaD (9 sections), every section computed, honest classification (computational claim, not formal theorem)', states: 'standard scientific format mapping, completed (user, 2026-07-26: "Standard scientific format mapping (begun, not complete yet)"). The renderer theoremAsStandardScientificPaper maps any theorem-fold to the FULL IMRaD paper — Title · Classification · Abstract · Methods · Results · Discussion · Reproducibility · References · Status — each section computed from the fold parts, with the honest classification (a verified computational claim, NOT a formal-logic theorem) baked in. The Results are refutable measurements each with a receipt, the Reproducibility is the merkle root so any reader recomputes, the Status is computed verified/refuted. HONEST: the mapping is now complete for the fold parts; clay=0. HARMONY ≠ TRUTH', provedBy: 'theoremAsStandardScientificPaperCompletesTheImradMappingEverySectionComputedWithHonestClassification', home: 'src/heaven/compute' },
