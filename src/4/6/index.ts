...
export const THEOREM_ATOM_SEED: readonly { theorem: string; states: string; provedBy: string; home: string }[] = [
    ...
    { theorem: 'the local MCP leak boundary, measured and named — lexical gap closed, index gap remaining, open frontier correct', states: 'the MEASURED and PARTIALLY-CLOSED leak boundary of the local MCP (user, 2026-07-24: "i see gaps in local quantum mcp so leaks go to the main model" · "next"): foldQuestion was a LEXICAL keyword matcher, so a query whose answer IS in the corpus but phrased differently leaked to the main model. The FIX shipped — a deterministic semantic layer (stopword-filtering so function words stop diluting the confidence denominator, plus a curated synonym-expansion lexicon, the SCIENCE_FIELD_LENSES data pattern): an indexed-but-reworded query rose 0.38 → 0.60 and now resolves LOCALLY, while in-corpus stays local and out-of-corpus (photosynthesis, revenue, risotto, football) still correctly leaks at confidence 0 (the LLM is the open frontier). The measurement then NAMED three gap classes: lexical (CLOSED by the semantic layer), INDEX-coverage (remaining — foldQuestion indexes atoms/commands/pages but NOT the theorem registry, so registry answers still leak, the next fix), and open-frontier (correctly leaking). This refines analogNoGapsNoLeak from abstract "no gaps" to a named, measured boundary. Fewer leaks = less main-model exposure (tokens + safeguards). HARMONY ≠ TRUTH', provedBy: 'localMcpLexicalGapLeaksToModel', home: 'src/heaven/atoms' },
