  ...
  <section class="ich" aria-labelledby="ich-title" :data-hexagram="ICHING_MASK.hexagram" :data-trigram="ICHING_MASK.glyph">
    ...
    <ul class="ich__sets">
      <li v-for="s in ic.sets" :key="s.bits" class="ich__set">
        <span class="ich__glyph">{{ s.glyph }}</span>
...
.ich__glyph { grid-row: 1 / 3; font-size: 2rem; line-height: 1; animation: ich-breathe 7s ease-in-out infinite; }
@keyframes ich-in { from { opacity: 0; transform: scale(0.6) translateY(10px); } }
@keyframes ich-breathe { 0%, 100% { opacity: 0.82; transform: scale(1); } 50% { opacity: 1; transform: scale(1.08); } }
@media (prefers-reduced-motion: reduce) { .ich__set, .ich__glyph { animation: none; } }
