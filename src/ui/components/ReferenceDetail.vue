...
<template>
  <div v-if="params">
    ...
    <p class="paper-nav">
      <a v-if="params.number > 1" :href="`${pfx}/references/r${String(params.number - 1).padStart(3, '0')}`">← Reference {{ params.number - 1 }}</a>
      <a :href="`${pfx}/references/`">All {{ params.total }} references</a>
      <a :href="`${pfx}/papers/${params.paperId}`">Paper {{ params.number }} →</a>
    </p>
  </div>
</template>
