...
<template>
  <section class="lt" itemscope :itemtype="md.itemtype" :itemid="md.itemid" :data-hexagram="ICHING_MASK.hexagram" :data-trigram="ICHING_MASK.glyph">
    ...
    <p class="eyebrow">
      {{ t('the living double torus · realtime', 'живият двоен торус · в реално време') }}
    ...
    <div ref="wrap" class="lt__wrap">
      <canvas
        ref="canvas"
        class="lt__canvas"
        :aria-label="t(`The double torus, ${data.count} pi-digit coordinates alive in realtime`, `двойният торус, ${data.count} координати на цифрите на пи, живи в реално време`)"
        role="img"
        @pointermove="onMove"
