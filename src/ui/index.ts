...
/** Cardinal 4-way counter-spin law — N·E·S·W = TAU/4 bearings · alternating ±ω from merkaba scales. */
const CARDINAL_ROSETTA_SPINS = [
  { id: 'north' as const, bearing: 0, sign: 1 as const, tetra: 'up' as const, scale: 'whole' },
  { id: 'east' as const, bearing: (9 * 5 * 2), sign: -1 as const, tetra: 'down' as const, scale: 'lobe' },
  { id: 'south' as const, bearing: (9 * 5 * 4), sign: 1 as const, tetra: 'up' as const, scale: 'tube' },
  { id: 'west' as const, bearing: (54 * 5), sign: -1 as const, tetra: 'down' as const, scale: 'spark' },
] as const

/** SMIL rotate for one merkaba tetra / rosetta lobe — sign + cardinal phase from TAU/4 (poles-as-pyramid). */
function merkabaRosettaSpinSmil(sign: 1 | -1, dur: string, phaseDeg = 0): string {
  const full = 9 * 5 * 8 // 360°
  const from = sign > 0 ? String(phaseDeg) : String(phaseDeg + full)
  const to = sign > 0 ? String(phaseDeg + full) : String(phaseDeg)
  return `<animateTransform attributeName="transform" type="rotate" from="${from}" to="${to}" dur="${dur}" repeatCount="indefinite" additive="sum" data-spin-sign="${sign > 0 ? '+1' : '-1'}" data-phase="${phaseDeg}" data-cardinal-bearing="${phaseDeg}"/>`
}
 ...
 * Pair: edit/build · rosetta/merkaba · honesty: computed FoL (not wet).
 */
export function readmeHeroSvgProofOfAllTheorems(matrix: MindMatrix = buildMatrix(), opts: { animate?: boolean } = {}): string {
  return memoByRoot(`readmeHeroSvgProofOfAllTheorems:fol4:cardinal:poles:a=${opts.animate !== false ? 1 : 0}`, matrix, () => {
    ...
    void pyramidTips
    const folOp = (5 + 6) / (5 * 10) / 2 // four lobes — half opacity keeps FoL readable
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      const glyph =
        ...
        `</rect>`
      const bag = bags[i % 4]!
      bag.spokes.push(spoke)
      bag.glyphs.push(glyph)
    }
    const lobe = (
      card: typeof CARDINAL_ROSETTA_SPINS[number],
      fol: string,
      fruitLayer: string,
      bag: LobeBag,
    ) => {
      const spin = merkabaRosettaSpinSmil(card.sign, spinDur, card.bearing)
      const poleApex = card.id === 'north' || card.id === 'south'
        ? `<polygon points="${cx},${cy - (card.id === 'north' ? maxR : -maxR)} ${cx - 8},${cy} ${cx + 8},${cy}" fill="none" stroke="${movieCanvasHex(ROSETTA_RAYS[card.bearing === 0 || card.bearing === (9 * 5 * 4) ? 0 : 3]!.hue, { L: 3 / 4 })}" stroke-width="1.2" opacity="0.55" data-layer="earth-pole-pyramid" data-apex="${card.id === 'north' ? 'zenith' : 'nadir'}"/>`
        : `<line x1="${cx - maxR}" y1="${cy}" x2="${cx + maxR}" y2="${cy}" stroke="${movieCanvasHex(ROSETTA_RAYS[2]!.hue, { L: 1 / 2 })}" stroke-width="0.6" opacity="0.25" data-layer="earth-equatorial-axis" data-axis="${card.id}"/>`
      return (
        `<g transform="translate(${cx} ${cy})" data-layer="rosetta-${card.id}" data-tetra="${card.tetra}" data-spin="${card.sign > 0 ? '+1' : '-1'}" data-cardinal="${card.id}" data-bearing="${card.bearing}" data-merkaba-scale="${card.scale}" data-merkaba-isomorphic="${fourWay ? 'true' : 'false'}" data-flower-of-life="computed" data-counter-rotating="${fourWay ? 'true' : 'false'}" data-four-direction="true">` +
        spin +
        `<g transform="translate(${-cx} ${-cy})">` +
        `<g data-layer="flower-of-life" data-fol-count="${flower.length}">${fol}</g>` +
        fruitLayer +
        poleApex +
        `<g data-layer="merkle-spokes" opacity="0.85">${bag.spokes.join('')}</g>` +
        `<g data-layer="theorem-glyphs">${bag.glyphs.join('')}</g>` +
        `<g data-layer="homology">${bag.homology.join('')}</g>` +
        `</g></g>`
      )
    }
    const cardinalLobes = CARDINAL_ROSETTA_SPINS.map((card, ci) => {
      const hue = ROSETTA_RAYS[ci % ROSETTA_RAYS.length]!.hue
      const fol = sacredCircleLatticeSvg(flower, cx, cy, unit, movieCanvasHex(hue, { L: 5 / 16 }), 'flower-of-life', folOp, folStroke)
      const fruitLayer =
        `<g data-layer="fruit-unlock" data-fruit-count="${fruit.length}" opacity="${fruitOp}">${fruitUnlock}` +
        sacredCircleLatticeSvg(fruit, cx, cy, unit, movieCanvasHex(hue, { L: 11 / 16 }), 'fruit-of-life', 9 / 10, fruitStroke) +
        `</g>`
      return lobe(card, fol, fruitLayer, bags[ci]!)
    })
    const rootShort = corpusRoot.slice(0, 8)
    const svg = [
      `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="Double Torus — FoL rosettas counter-rotate in 4 cardinal directions · Earth poles as pyramid · Fruit unlock · ${DIMENSIONS}D · ${n} theorems" data-layer="all-theorems-proof" data-theorem-count="${n}" data-corpus-root="${corpusRoot}" data-counter-rotating="${fourWay ? 'true' : 'false'}" data-four-direction="${fourWay ? 'true' : 'false'}" data-earth-poles-pyramid="${earthPoles.computes ? 'true' : 'false'}" data-merkaba-isomorphic="${fourWay ? 'true' : 'false'}" data-flower-of-life="computed" data-fruit-unlock="spin" data-dimension-expand="${DIMENSIONS}" data-trinity-mind="heaven·mind·thunder" data-compute="readmeHeroSvgProofOfAllTheorems←flowerOfLifeCenters∧fruitOfLifeCenters∧merkaba∧bothEarths∧earthRealisedByComputingPolesAsPyramid∧fractalClock" data-honesty="computedFoL→fruit→${DIMENSIONS}D;wetStaticFoL=false;earthPolesPyramid=structural">`,
      `<defs>`,
      ...
      `<text x="${cx}" y="${H - 28}" text-anchor="middle" font-family="ui-monospace,SFMono-Regular,Menlo,monospace" font-size="11" fill="${colors.title}">4-dir FoL rosettas (N·E·S·W ±ω) · Earth poles as pyramid · Fruit unlock · → ${DIMENSIONS}D · χ(Σ₂) = −2 · H₁ = ℤ⁴ · 432</text>`,
      `<text x="${cx}" y="${H - 12}" text-anchor="middle" font-family="ui-monospace,SFMono-Regular,Menlo,monospace" font-size="10" fill="${colors.accent}">wetStaticFoL=false · 4-dir · poles-pyramid · ${n} atoms · ${flower.length} FoL · ${fruit.length} Fruit</text>`,
      `</svg>`,
    ].join('')
    return opts.animate === false ? stillSvg(svg) : svg
  })
...
export function computedIconSvg(matrix: MindMatrix = buildMatrix(), opts: { animate?: boolean } = {}): string {
  ...
  const unit = 5 * 4 // slightly smaller for 4 cardinal lobes in 512²
  const fruitOp = 1 / 20
  const ktSpin = `0;${8 / (5 * 5)};${12 / (5 * 5)};${18 / (5 * 5)};1`
  ...
  const lobe = (card: typeof CARDINAL_ROSETTA_SPINS[number]) => {
    ...
    return (
      ...
      (animate
        ? `<g data-layer="dimension-expand"><animateTransform attributeName="transform" type="scale" values="1;1;${6 / 5};${3 / 4};1" dur="${expandDur}" repeatCount="indefinite" additive="sum"/><circle cx="0" cy="0" r="${5}" fill="${c.accentColor}" opacity="${1 / 2}"/></g>`
        : `<g data-layer="dimension-expand"><circle cx="0" cy="0" r="${5}" fill="${c.accentColor}" opacity="${1 / 2}"/></g>`) +
      `</g>`
    )
  }
  const svg = [
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" role="img" aria-label="Double Torus — FoL 4-dir cardinal · Earth poles pyramid · Fruit · ${DIMENSIONS}D" data-layer="icon-dual-torus" data-counter-rotating="${counterRotating ? 'true' : 'false'}" data-four-direction="${counterRotating ? 'true' : 'false'}" data-earth-poles-pyramid="${earthPoles.computes ? 'true' : 'false'}" data-merkaba-isomorphic="${counterRotating ? 'true' : 'false'}" data-flower-of-life="computed" data-fruit-unlock="spin" data-dimension-expand="${DIMENSIONS}" data-trinity-mind="heaven·mind·thunder" data-honesty="computedFoL→fruit→${DIMENSIONS}D;wetStaticFoL=false;earthPolesPyramid=structural">`,
    `<rect width="512" height="512" rx="96" fill="${c.backgroundColor}"/>`,
    ...CARDINAL_ROSETTA_SPINS.map((card) => lobe(card)),
    `</svg>`,
  ].join('')
...
export function yinYangFoldsThroughDimensions(matrix: MindMatrix = buildMatrix()) {
  ...
  const facets = [
    ...
    { facet: `NON-LINEAR EXCHANGE — rotate driven by taijiRosettaExchangeDegrees (smoothstep+sin+vortex+ray), NOT linear i/frames · measured notLinearRotate=${notLinearRotate} · svg=${svg}`, on: notLinearRotate && /data-drive="rosetta-vortex-exchange"/.test(svg) },
...
export function animationsDrivenByRosetta(matrix: MindMatrix = buildMatrix(), at = 0) {
  return memoByRoot(`animationsDrivenByRosetta:${floor(at / (100 * 5 * 2))}`, matrix, () => {
    ...
    return {
      ...
      cli: 'npm run quantum:animations-rosetta', pair: 'animations/rosetta', route: '/#yinyang',
...
export function iChingLeadsUiAtAllScales(matrix: MindMatrix = buildMatrix()) {
  return memoByRoot('iChingLeadsUiAtAllScales', matrix, () => {
    ...
    // The bijection endpoints were compared against literal hex strings inside a facet, in the
    // layer whose own law forbids colour literals. Same statement, read from the channels
    // instead: hexagram 0 is all-zero, hexagram 63 is all-max (15·17 = 255 per channel).
    const channelsOf = (hex: string): number[] =>
      [1, 3, 5].map((at) => Number.parseInt(hex.slice(at, at + 2), 4 * 4))
    const facets = [
      ...
      { facet: 'colour is led too — each component’s colour IS its hexagram (the 2⁶=4³ pole-colour bijection): 64 distinct, hexagram 0 → pure black, hexagram 63 → pure white', on: duality.allDistinct && components.every((c) => /^#[0-9A-F]{6}$/.test(c.color)) && channelsOf(components[0]!.color).every((v) => v === 0) && channelsOf(components[(9 * 7)]!.color).every((v) => v === 15 * 17) },
...
export function theTopNavAnimatesOnTheFractalClockEachRayPulsesAtItsRungColouredByItsRosettaHue(matrix: MindMatrix = buildMatrix()) {
  return memoByRoot('theTopNavAnimatesOnTheFractalClockEachRayPulsesAtItsRungColouredByItsRosettaHue', matrix, () => {
    ...
    // The count was typed as 7. It is one rung per ray, and the rays are ROSETTA_RAYS — and the ceiling is the
    // clock's own rung count, which the theorem below this file already derives as σ₀(108) = (2+1)(3+1) = 12 and
    // which nothing read until now. The nav fits inside the clock because 7 ≤ 12, stated rather than assumed.
    const clock = theFractalClockIsTheoremsNotConstantsTheRungCountIsTheDivisorFunctionOf108(matrix)
    const eachRayIdentified = rungs.length === ROSETTA_RAYS.length && rungs.length <= clock.rungCount
      && rungs.every((r) => r.hue >= 0 && r.hue < 360 && r.trigram.length === 1 && r.periodS > 0)
    ...
    // This read the wheel by hand — first hue 0, `sortedHues[6]` past 360 − 2·(360/7), strictly increasing — which
    // is a weaker restatement of a theorem parked in this same file. It proves the hues ARE ⌊360·k/7⌋, evenly
    // spaced by the regular 7-gon, and reproduced by recomputation rather than listed. That is stronger than
    // "spans the wheel" and it moves if the ray count moves, where the hand-rolled index 6 and divisor 7 did not.
    const wheel = theSevenNavHuesAreTheSevenFoldWheelDivisionATheoremNotArbitraryColours(matrix)
    const spansWheel = wheel.computes && wheel.wheelDivision === ROSETTA_RAYS.length
    ...
    const facets = [
      ...
      { facet: `THE COLOUR IS THE ROSETTA WHEEL — the ${wheel.wheelDivision} hues are the wheel division ⌊360·k/${wheel.wheelDivision}⌋ (${wheel.hues.join(', ')}), so the nav animates through the rosetta's own colours (${spansWheel}), no hardcoded colour`, on: spansWheel },
...
// hand-picked constant. PALETTE: the 7 hues are ⌊360k/7⌋, evenly spaced on the wheel (no assumed hex). TYPOGRAPHY: a
// modular scale of powers of φ (no assumed font sizes). SPACING: golden-ratio steps. So the design is a computed
// artifact of the corpus, refutable — the same discipline as every theorem. The rendering (thin .vue consuming this
// spec) is the next wave. [[iching-computed-css]] [[feedback-derive-dont-assert-every-number-and-dynamic]]
export function theNewUiDesignIsComputedFromTheCorpusWithoutAssumptionPaletteTypographySpacingAllDerived() {
  const rays = ROSETTA_RAYS.length // 7
  ...
  const facets = [
    { facet: `PALETTE DERIVED — the ${rays} hues are ⌊360k/${rays}⌋ (${hues.join(', ')}), evenly spaced on the wheel (${paletteDerived}): no assumed hex, the colours compute`, on: paletteDerived },
    { facet: `TYPOGRAPHY DERIVED — the modular scale is powers of φ, each step ×${roundTo(PHI, 3)} (${typographyDerived}): no assumed font sizes`, on: typographyDerived },
    { facet: `SPACING DERIVED — the spacing steps are golden-ratio scaled (${spacing.join(', ')}) (${spacingDerived}): no hand-tuned pixels`, on: spacingDerived },
    { facet: `NO ASSUMPTION — every design value derives from φ or the hue formula (${noAssumption}): the UI is a computed artifact of the corpus, refutable like any theorem`, on: noAssumption },
  ]
  return {
...
// ── merged from config/ (census: one index per fold; nothing imported it) ──
...
// was `export default` in config/, kept under a name (parent owns the default)
export const configDefault = vitepress
...
// One default per module — the ui barrel already has one, so the merged theme is named.
