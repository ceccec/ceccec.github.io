...
| docs:build wall-clock | buildMs (warm path when cached) | 196000 | 123151 | 1.592× vs CI baseline | `buildMin` |
...
- Receipt: fold `qpuCpuGpu` · root=236d3a02 · apparentFtl=0.147×.
...
gateLight — coverage=0.7163 gateCost=367 buildNorm=0.628 inverse=1.1587 buildSpeedup=1.592× factsOnce=1.
...
| buildMs | 123151 | ↓ vs baseline 196000ms | `buildMin timing` |
| deployMs | 11000 | ↓ warm CI cache path | `buildMin CI baseline` |
| inverseScore | 1.1587 | → drain manual · on:true debt | `gateLight law receipt` |
| factsOnce | 1 | collectEnforcementFacts once · wave/verify subtracts render | `trinity/speedup · wave/verify` |

inverseRelationOn=false · computeCoverage=0.7163 · gateCost=367 · buildSpeedup=1.592×
