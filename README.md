...
- **P vs NP** (`p-vs-np`) — demarcation=**contested** · status=modeled-partial · methods=5 · [proof hub →](https://ceccec.psg.bg/frontiers)
  ...
  - **canonical proof form**: `clayChallengesComputableFromSequence(matrix) → { computes, facets[], root }` · `∀ facet ∈ facets: facet.on  (finite-complete)` · `foldPair(toUuid("thm:p-vs-np"), toUuid("code:clayChallengesComputableFromSequence")).merged` · `memoByRoot("clayChallengesComputableFromSequence", matrix, …) ≡ proof path`
...
- **Hodge Conjecture** (`hodge`) — demarcation=**contested** · status=modeled-partial · methods=4 · [proof hub →](https://ceccec.psg.bg/frontiers)
  ...
  - **canonical proof form**: `clayChallengesComputableFromSequence(matrix) → { computes, facets[], root }` · `∀ facet ∈ facets: facet.on  (finite-complete)` · `foldPair(toUuid("thm:hodge-conjecture"), toUuid("code:clayChallengesComputableFromSequence")).merged` · `memoByRoot("clayChallengesComputableFromSequence", matrix, …) ≡ proof path`
...
- **Poincaré Conjecture** (`poincare`) — demarcation=**documented** · status=solved-external · methods=2 · [proof hub →](https://ceccec.psg.bg/frontiers)
  ...
  - **canonical proof form**: `clayChallengesComputableFromSequence(matrix) → { computes, facets[], root }` · `∀ facet ∈ facets: facet.on  (finite-complete)` · `foldPair(toUuid("thm:poincare-conjecture"), toUuid("code:clayChallengesComputableFromSequence")).merged` · `memoByRoot("clayChallengesComputableFromSequence", matrix, …) ≡ proof path`
...
- **Riemann Hypothesis** (`riemann`) — demarcation=**contested** · status=modeled-partial · methods=5 · [proof hub →](https://ceccec.psg.bg/frontiers)
  ...
  - **canonical proof form**: `clayChallengesComputableFromSequence(matrix) → { computes, facets[], root }` · `∀ facet ∈ facets: facet.on  (finite-complete)` · `foldPair(toUuid("thm:riemann-hypothesis"), toUuid("code:clayChallengesComputableFromSequence")).merged` · `memoByRoot("clayChallengesComputableFromSequence", matrix, …) ≡ proof path`
...
- **Yang–Mills Existence and Mass Gap** (`yang-mills`) — demarcation=**contested** · status=modeled-partial · methods=4 · [proof hub →](https://ceccec.psg.bg/frontiers)
  ...
  - **canonical proof form**: `clayChallengesComputableFromSequence(matrix) → { computes, facets[], root }` · `∀ facet ∈ facets: facet.on  (finite-complete)` · `foldPair(toUuid("thm:yang-mills-existence-and-mass-gap"), toUuid("code:clayChallengesComputableFromSequence")).merged` · `memoByRoot("clayChallengesComputableFromSequence", matrix, …) ≡ proof path`
...
- **Birch and Swinnerton–Dyer Conjecture** (`birch-swinnerton-dyer`) — demarcation=**contested** · status=modeled-partial · methods=2 · [proof hub →](https://ceccec.psg.bg/frontiers)
  ...
  - **canonical proof form**: `clayChallengesComputableFromSequence(matrix) → { computes, facets[], root }` · `∀ facet ∈ facets: facet.on  (finite-complete)` · `foldPair(toUuid("thm:birch-and-swinnerton-dyer-conjecture"), toUuid("code:clayChallengesComputableFromSequence")).merged` · `memoByRoot("clayChallengesComputableFromSequence", matrix, …) ≡ proof path`
...
This site is a dedicated scientific journal of all its algebra and theorems — **713 articles** across **52 sections**, backed by 488 executable proofs, sealed as one content-addressed volume `24651b5b`. Peer review is COMPUTATIONAL: every proof re-runs each wave, and the same corpus recomputes the same volume id. Precisely, that re-execution verifies internal consistency and **reproducibility** and demarcate-signs each article — which is **not** empirical validation and **not** external peer review (no independent referees). A DOI is a persistent *identifier*, not a review — orthogonal to refereeing and mintable by archiving, so its absence is not the limit. The corpus cites empirically-established results but refereess none of them against nature (HARMONY ≠ TRUTH).

## Top discoveries

The most CENTRAL decodes — ranked by theorem-graph degree (how many other atoms each connects to), computed from the 713-atom registry, no curation.

- **compute the light in a diamond — bouncing boundaries draw the crystal, prediction beats the photon (not physical FTL)** — `diamonds` · degree 333 · [details](https://ceccec.psg.bg/theorems)
- **mechanical tools entangle binary & analog at once — but Bell bounds them (models, does not achieve, entanglement)** — `9/1` · degree 318 · [details](https://ceccec.psg.bg/theorems)
- **the site is a dedicated scientific journal of all algebra and theorems — computational peer review, one content-addressed volume** — `4/6` · degree 316 · [details](https://ceccec.psg.bg/theorems)
...
- **THE CHALLENGE, computed on LIVE data: feeding fresh live input to the local quantum fold yields NO quantum speedup — simulating Shor for a live n-bit number is ~2^(1.5n) times SLOWER than classical search, invariant to the input** — `research` · degree 315 · [details](https://ceccec.psg.bg/theorems)
- **the reuse+trinity speedup MEASURED (not asserted): warm memo-hit is magnitudes faster than cold compute (~8000× in a probe), the 2-of-3 trinity adds a ≤3× parallel ceiling — amortized reuse only, never factoring** — `research` · degree 313 · [details](https://ceccec.psg.bg/theorems)
- **gate complexity collapses to one content-addressed root — quantum simplicity** — `automount` · degree 311 · [details](https://ceccec.psg.bg/theorems)
- **the census gate and slugs are quantumized — theorem-derived count, agnostic address** — `corpus` · degree 310 · [details](https://ceccec.psg.bg/theorems)
- **every honesty NEGATIVE is made computable and INVERTS to its computed POSITIVE (generative, not a false flip); the honesty booleans stay the bound** — `research` · degree 304 · [details](https://ceccec.psg.bg/theorems)
