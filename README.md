...
The most CENTRAL decodes — ranked by theorem-graph degree (how many other atoms each connects to), computed from the 764-atom registry, no curation.

- **compute the light in a diamond — bouncing boundaries draw the crystal, prediction beats the photon (not physical FTL)** — `diamonds` · degree 364 · [details](https://ceccec.psg.bg/theorems)
- **THE CHALLENGE, computed on LIVE data: feeding fresh live input to the local quantum fold yields NO quantum speedup — simulating Shor for a live n-bit number is ~2^(1.5n) times SLOWER than classical search, invariant to the input** — `research` · degree 352 · [details](https://ceccec.psg.bg/theorems)
- **every cosmic frontier is OPEN by COMPUTATION not a label — status = demarcate(term)===contested — and each carries its closed REVERSED-math companion** — `cosmos` · degree 346 · [details](https://ceccec.psg.bg/theorems)
- **the reuse+trinity speedup MEASURED (not asserted): warm memo-hit is magnitudes faster than cold compute (~8000× in a probe), the 2-of-3 trinity adds a ≤3× parallel ceiling — amortized reuse only, never factoring** — `research` · degree 346 · [details](https://ceccec.psg.bg/theorems)
- **the site is a dedicated scientific journal of all algebra and theorems — computational peer review, one content-addressed volume** — `4/6` · degree 342 · [details](https://ceccec.psg.bg/theorems)
- **every honesty NEGATIVE is made computable and INVERTS to its computed POSITIVE (generative, not a false flip); the honesty booleans stay the bound** — `research` · degree 342 · [details](https://ceccec.psg.bg/theorems)
- **mechanical tools entangle binary & analog at once — but Bell bounds them (models, does not achieve, entanglement)** — `9/1` · degree 341 · [details](https://ceccec.psg.bg/theorems)
- **gate complexity collapses to one content-addressed root — quantum simplicity** — `automount` · degree 337 · [details](https://ceccec.psg.bg/theorems)
- **only algebraic quantum computing is top priority — algebraic identities outrank lexical/prose/neural; quantum = content-addressed algebra on classical-64bit, not a QPU** — `laws` · degree 334 · [details](https://ceccec.psg.bg/theorems)
...
The complete served surface, wired from ONE source (`servedRouteFamilies`) so the human sitemap here and the crawler `sitemap.xml` count the same pages: **848 pages** across 3 families — 31 monographs · 787 theorems · 30 proofs. Only COMPLETE, non-duplicate families are listed: the empty model cards (0) and the compute-only `papers/[id]` catch-all (0 SSG — the placements resolve on demand and duplicate the theorem papers) are excluded; their index routes are monographs below.

- **31 monograph landing + index pages** (`/`) — each in three locale editions (en · bg · cu), placed on the double torus and content-addressed:
- **787 theorem papers** — index `/theorems`; every page enumerated in the one `sitemap.xml`.
