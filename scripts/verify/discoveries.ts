...
import { leanPagePaths, leanPageSlug, leanTheoremsForLatex } from '../../src/pair/formal/proofs/index.ts'
...
export function assertDepositsAreHonest(): void {
  ...
  // A LEFTOVER BUILD IS NOT EVIDENCE EITHER (2026-09-14). dist is whatever the LAST docs:build left, and the
  // stream never builds — so this graded a previous build. A new theorem failed because its page did not exist
  // yet (spacetime-the-reverse-boost-undoes-the-boost), and worse, a theorem whose page had been REMOVED would
  // have passed on the stale file. The verdict now comes from leanPagePaths(), the one function the
  // /lean/[slug] route generates its pages from: it is computed on every run from the sources, so it is never
  // stale and never absent. dist is secondary evidence, read only when it is newer than every source the
  // records point at; a stale dist is reported as not read, and it neither passes nor fails anything.
  const generated = new Set(leanPagePaths(process.cwd()).map((p) => p.params.slug))
  const unrouted = records.filter((r) => !generated.has(r.landingPage.slice(`${SITE}/lean/`.length)))
  if (unrouted.length) throw new Error(`${unrouted.length} deposit(s) name a landing page the /lean/ route does not generate: ${unrouted.slice(0, 3).map((r) => r.landingPage).join(', ')}`)
  console.log(`  ${records.length}/${records.length} landing pages are generated by the /lean/ route — computed from the sources, not read from a build`)
  const indexHtml = join(process.cwd(), '.vitepress/dist/index.html')
  if (!existsSync(indexHtml)) {
    console.log(`  dist not read — no completed build in .vitepress/dist (no index.html); the verdict above stands on the sources`)
    return
  }
  const newestSource = Math.max(...[...paths].map((rel) => statSync(join(process.cwd(), rel)).mtimeMs))
  if (statSync(indexHtml).mtimeMs < newestSource) {
    console.log(`  dist not read — the build is older than the sources it would have to show; the verdict above stands on the sources`)
    return
  }
  ...
  console.log(`  and ${records.length}/${records.length} exist in the build, which is newer than every source`)
}
