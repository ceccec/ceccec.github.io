...
import { componentGraph, harmonicBands, foldedCensus, folderLaw, jsonLdPathRules, buildEnforcementPipeline, zeroTokenPolicy, staticPages, quantumSitemap, doubleTorusWords, terabyteEncryptionInMegabyteCodebase } from '../src/ui/lib/quantumMind.ts'

const root = process.cwd()
const contentRoot = join(root, 'site') // the page tree (srcDir): every locale (gla/en/bg) lives under site/; the root holds only config, tooling and src
...
for (const [route, components] of Object.entries(placedBy)) {
  ...
  const en = read(join(contentRoot, file))
  const bg = read(join(contentRoot, 'bg', file))
  for (const component of components) {
...
const generatedRoot = new Set(['node_modules', 'gla']) // node_modules from npm; gla/ is the Glagolitic locale, generated each build by generate-glagolitic from the English pages
...
// below the English root. The page tree is rooted at site/ (srcDir); its locale roots (gla at site/,
// en at site/en, bg at site/bg) and the folders below them are unchanged, so the law walks site/'s children.
...
for (const marker of ['root monograph', 'Computed from src — do not edit by hand', 'Reproducibility']) {
  if (!readmeText.includes(marker)) {
    gaps.push({ harmonic: 'monograph', kind: 'readme-not-computed', detail: `README.md lacks the computed marker "${marker}" — why this fails: the README is the root monograph generated by scripts/generate-readme.mjs from the one source; a hand-kept README is entropy that drifts from the model — run the build to regenerate it` })
    break
  }
}
...
console.log('Gates enforce all: entropy does not pass — mirroring, orphan routes, a hand-kept README, drift and redundancy are all refused; every gate names its fix, so a caught gap is recycled into the next fold.')
