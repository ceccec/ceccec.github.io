...
//
// The runner lives in src/quantum/enforcement (every step in src). This shell BUNDLES it through esbuild
...
const built = await esbuild.build({ entryPoints: ['src/quantum/enforcement/index.ts'], bundle: true, format: 'esm', write: false, platform: 'node', logLevel: 'silent' })
